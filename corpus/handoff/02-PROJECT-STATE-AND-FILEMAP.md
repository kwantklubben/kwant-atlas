...
- **Next target:** **Fundamentals & Accounting — DONE.** All 8 topic-folders built + lockstep pass committed: financial-statements-and-accounting, core-financial-ratios, equity-valuation, fundamental-analysis-and-screening, accounting-quality-and-red-flags, capital-structure-and-corporate-finance, quantitative-fundamental-investing, data-sources-and-corporate-data. Area index + home index link + visualizer nodes + README done. Build clean (264 files).
- **Next target:** **Pillar 1 (Quantitative Research) — DONE.** 10 topic-folders built + lockstep + audit + fixes committed: statistical-arbitrage-and-pairs, backtesting-hygiene, fundamental-multi-factor-models, momentum, signal-processing-and-kalman, feature-engineering-and-labeling, event-studies, regime-detection, garch-and-volatility-modeling, factor-investing-and-timing. Audit verdict PASS-with-gaps; the 2 gaps it found (GARCH, factor-timing) were then built; code/output mismatch + notation fixed. Visualizer node cluster added.
- **Next target:** **Pillar 4 (Quantitative Risk) — DONE.** 11 topic-folders built + lockstep committed: var-and-expected-shortfall, parametric-historical-and-monte-carlo-var, extreme-value-theory-and-fat-tails, credit-risk-and-the-merton-model, stress-testing-and-scenario-analysis, liquidity-risk-and-funding, counterparty-risk-and-xva, model-risk-and-validation, basel-and-regulation, operational-risk, risk-factor-sensitivities. Audit pending.
- **In progress:** Pillar 5 (Portfolio Optimization) — **DONE + AUDITED**. 9 topic-folders; audit PASS-with-minor-fixes (fixed: 2-asset ERC closed form transposed subscripts, garbled tangency identity, 4 dangling links, flat notes → `_legacy/`, ~85 file links repointed to folder hubs).
...
- **In progress:** Pillar 6 (Market Making) — **DONE** (9 topic-folders: limit-order-book-mechanics, avellaneda-stoikov-and-optimal-quoting, adverse-selection-and-glosten-milgrom, spread-decomposition-and-roll-model, inventory-management-and-quote-skewing, toxic-order-flow-and-vpin, market-impact-and-depth, liquidity-risk-and-asset-pricing, market-maker-economics-and-rebates). Lockstep + audit running.
...
- **In progress:** Pillar 2 (Algorithmic Trading & HFT) — 4 committed (optimal-execution-and-almgren-chriss, execution-algorithms-vwap-twap-pov, queue-position-and-fill-probability, market-microstructure-and-order-types). Building: smart-order-routing-and-fragmentation, execution-backtesting-and-simulation, colocation-and-clock-synchronization, hardware-acceleration-and-fpga, low-latency-systems-architecture (→9). Target: then lockstep + audit.
- **In progress:** Pillar 8 (Quant Dev, dir `08-quantitative-development`) — started: high-performance-cpp-for-trading building. Target ~10 folders (low-latency Linux, tick DBs, event-driven backtesting, FIX, lock-free, FPGA/GPU, Python stack, data infra, production systems).
- **Then:** Pillar 8 completion → lockstep → audit; final full-repo build + comprehensive status; merge branch to main.
