...
- **In progress:** Pillar 2 (Algorithmic Trading & HFT) — 1 folder committed (optimal-execution-and-almgren-chriss). Building: execution-algorithms-vwap-twap-pov, queue-position-and-fill-probability. Target ~9 folders (also: market-microstructure-and-order-types, smart-order-routing, colocation-and-clock-sync, hardware-acceleration-fpga, low-latency-systems-architecture, execution-backtesting-and-simulation).
- **Remaining pillar:** 8 (Quant Dev, dir `08-quantitative-development`, 7 flat notes → ~10 folders). Then final audits. Each built via the same folder-per-topic + lockstep pattern.
