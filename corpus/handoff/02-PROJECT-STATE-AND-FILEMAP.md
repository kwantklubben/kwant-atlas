...
- **Next:** scale the folder-per-topic pattern to another area. **Foundations is the natural next target** (rich verified corpus: linear algebra, calculus, probability/measure, stochastic calculus, econometrics/timeseries, numerics, plus free books Axler/Boyd/Durrett/MacKay on disk). Then Fundamentals & Accounting, then the other pillars.
