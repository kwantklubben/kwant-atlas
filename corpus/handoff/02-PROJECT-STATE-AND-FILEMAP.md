...
- **Next target:** the remaining operational pillars (1, 2, 4-8) which have corpus + free/paid sources ready. Suggest order: Pillar 1 (quant research) and Pillar 4 (risk) have the strongest verified corpora; then 5 (portfolio), 7 (ML), 6 (market making), 2 (HFT), 8 (quant dev). Each built via the same folder-per-topic + lockstep pattern.
