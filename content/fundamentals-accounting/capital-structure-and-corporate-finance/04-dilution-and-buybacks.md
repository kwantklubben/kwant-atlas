...
```python
print("== Dilution: issuing shares BELOW intrinsic value (info asymmetry) ==")
V, n = 1000.0, 100.0          # firm value, shares -> $10/share intrinsic
p, m = 8.0, 25.0              # issue at $8 (< $10) to raise $200
V_new = V + p*m
per_share = V_new/(n+m)
old_after = per_share*n
transfer = V - old_after
print(f"intrinsic/share={V/n:.2f}; issue at p={p:.2f} -> V_new={V_new:.0f}, "
      f"shares={n+m:.0f}, per-share={per_share:.2f}")
print(f"OLD holders: had {V:.0f}, now {old_after:.0f} -> wealth transfer "
      f"{transfer:.0f} to NEW holders")

print()
print("== Buyback: EPS accretion vs. genuine value creation ==")
ebit, T = 200.0, 0.30
ni = ebit*(1-T)
sh, price = 100.0, 10.0
buy_back = 10.0               # retire 10 shares for $100 cash
sh2 = sh - buy_back
print(f"Before: NI={ni:.0f}, shares={sh:.0f}, EPS={ni/sh:.2f}")
print(f"After 10-share buyback: shares={sh2:.0f}, EPS={ni/sh2:.3f} "
      f"(EPS up {(ni/sh2)/(ni/sh)*100-100:.1f}%!)")
# The honest lens: the $100 cash was an earning asset at ROA 10%
ebit_lost = 100.0*0.10*(1-T)
print(f"BUT the $100 cash earned at ROA 10%: forgone NI = {ebit_lost:.1f} -> "
      f"EPS = {(ni-ebit_lost)/sh2:.3f} (accretion mostly evaporates)")
```

```
== Dilution: issuing shares BELOW intrinsic value (info asymmetry) ==
intrinsic/share=10.00; issue at p=8.00 -> V_new=1200, shares=125, per-share=9.60
OLD holders: had 1000, now 960 -> wealth transfer 40 to NEW holders

== Buyback: EPS accretion vs. genuine value creation ==
Before: NI=140, shares=100, EPS=1.40
After 10-share buyback: shares=90, EPS=1.556 (EPS up 11.1%!)
BUT the $100 cash earned at ROA 10%: forgone NI = 7.0 -> EPS = 1.478 (accretion mostly evaporates)
```
