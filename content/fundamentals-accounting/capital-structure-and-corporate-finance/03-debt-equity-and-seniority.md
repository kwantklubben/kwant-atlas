...
```python
# Priority waterfall: each claim recovers min(claim, remaining assets).
claims = [("senior debt", 300.0), ("junior debt", 200.0), ("mezzanine", 100.0),
          ("convertible", 100.0), ("common equity", 300.0)]
total = sum(a for _, a in claims)
print("Priority waterfall of $%g total claims against liquidation value" % total)
print("scenario   " + "  ".join(c[:4] for c, _ in claims) + "   equity=residual")
for assets in (900.0, 650.0, 480.0, 250.0, 100.0):
    remaining = assets; recov = []
    for name, amt in claims:
        pay = min(remaining, amt); recov.append(pay); remaining -= pay
    eq = recov[-1]
    print(f"  {assets:5.0f}    " + "  ".join(f"{p:5.1f}" for p in recov) +
          f"   equity gets {eq:.1f}")
```

```
Priority waterfall of $1000 total claims against liquidation value
scenario   seni  juni  mezz  conv  comm   equity=residual
    900    300.0  200.0  100.0  100.0  200.0   equity gets 200.0
    650    300.0  200.0  100.0   50.0    0.0   equity gets 0.0
    480    300.0  180.0    0.0    0.0    0.0   equity gets 0.0
    250    250.0    0.0    0.0    0.0    0.0   equity gets 0.0
