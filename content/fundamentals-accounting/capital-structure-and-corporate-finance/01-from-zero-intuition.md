...
```python
# One project, three funding mixes: watch leverage amplify (and invert) ROE.
K = 1000.0                                   # total capital funding the project
states = {"good": 300.0, "bad": 60.0}        # EBIT in each state
rd = 0.08
print("EBIT state | 100% eq | 50% eq/50% debt | 20% eq/80% debt")
for s, ebit in states.items():
    row = []
    for eq_frac in (1.0, 0.5, 0.2):
        eq = K*eq_frac; debt = K - eq
        row.append((ebit - debt*rd)/eq)
    print(f"  {s:9s}| {row[0]*100:7.2f}% | {row[1]*100:13.2f}% | {row[2]*100:15.2f}%")
```

```
EBIT state | 100% eq | 50% eq/50% debt | 20% eq/80% debt
  good     |   30.00% |         52.00% |          118.00%
  bad      |    6.00% |          4.00% |           -2.00%
```
