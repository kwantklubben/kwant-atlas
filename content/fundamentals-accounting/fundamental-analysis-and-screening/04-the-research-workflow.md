...
```python
# Filing red-flag scanner: the numbers you check after the screen, from a 10-K/10-Q.
filing = dict(rev_growth=0.14, recv_growth=0.31, inv_growth=0.05, cfo=90.0, ni=105.0,
              auditor_changed=False, restatement=True, related_party=True,
              insider_buying=True, dso_days=58, dso_days_prior=42)
flags = []
if filing['recv_growth'] > filing['rev_growth'] + 0.10:
    flags.append("Receivables outrunning revenue -> channel stuffing / weak collections")
if filing['dso_days'] - filing['dso_days_prior'] > 10:
    flags.append(f"DSO jumped {filing['dso_days']-filing['dso_days_prior']} days")
if filing['cfo'] / filing['ni'] < 0.8:
    flags.append("CFO/NI < 0.8 -> accrual-heavy earnings")
if filing['auditor_changed']:
    flags.append("Auditor changed -> investigate why")
if filing['restatement']:
    flags.append("Restatement -> prior numbers unreliable")
if filing['related_party']:
    flags.append("Related-party transactions -> check arm's length")
positives = ["Insider buying (management skin in the game)"] if filing['insider_buying'] else []
print("RED FLAGS:")
for f in flags: print("  !", f)
print("GREEN FLAGS:")
for p in positives: print("  +", p)
print(f"Net flag balance = {len(positives)} green vs {len(flags)} red")
```
```
RED FLAGS:
  ! Receivables outrunning revenue -> channel stuffing / weak collections
  ! DSO jumped 16 days
  ! Restatement -> prior numbers unreliable
  ! Related-party transactions -> check arm's length
GREEN FLAGS:
  + Insider buying (management skin in the game)
Net flag balance = 1 green vs 4 red
```
