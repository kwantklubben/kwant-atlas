...
```python
# Graham's deeper tools: NCAV net-net floor, earnings-power value, enterprising caps.
ca, tot_liab, shares_m = 180.0, 90.0, 10.0      # current assets, ALL liabilities, shares (m)
ncav_total = ca - tot_liab
ncav_ps = ncav_total / shares_m
price = 9.0
print(f"Net-current-asset value (NCAV) total = {ncav_total:.0f}M  -> per share = {ncav_ps:.2f}")
print(f"Graham net-net buy rule  price <= 2/3*NCAV = {2/3*ncav_ps:.2f}  "
      f"-> {'NET-NET BUY' if price <= 2/3*ncav_ps else 'not a net-net'}")
norm_earn, r = 18.0, 0.10                        # normalized (mid-cycle) earnings, cost of capital
epv = norm_earn / r
print(f"Earnings-power value = {norm_earn}/{r} = {epv:.0f}M  -> EPV/share = {epv/shares_m:.2f}")
print(f"Price {price:.2f} vs NCAV/share {ncav_ps:.2f}: asset floor "
      f"{'above' if price<ncav_ps else 'below'} price")
eps, bvps = 1.20, 7.0                            # enterprising caps: P/E<=10, P/B<=1.5
print(f"Enterprising test: P/E={price/eps:.1f}<=10:{price/eps<=10.0}  "
      f"P/B={price/bvps:.2f}<=1.5:{price/bvps<=1.5}")
```
```
Net-current-asset value (NCAV) total = 90M  -> per share = 9.00
Graham net-net buy rule  price <= 2/3*NCAV = 6.00  -> not a net-net
Earnings-power value = 18.0/0.1 = 180M  -> EPV/share = 18.00
Price 9.00 vs NCAV/share 9.00: asset floor below price
