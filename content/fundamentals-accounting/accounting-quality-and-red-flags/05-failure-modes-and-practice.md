...
```python
# Four failure modes of quality screens, each reduced to its mechanism.
# --- mode 1: accrual reversal -- the same +60 flips sign next year
y1_acc, y1_cf = 60.0, 40.0
print("--- mode 1: accrual reversal (stuffing today = negative accruals tomorrow)")
print(f"   Y1 earnings = {y1_acc + y1_cf:.0f}  (accruals {y1_acc:+.0f}, cash {y1_cf:+.0f})")
y2_acc = -y1_acc                                  # the receivable unwinds
print(f"   Y2 earnings = {y2_acc + y1_cf:.0f}  (accruals {y2_acc:+.0f}, cash {y1_cf:+.0f})")
print("   A naive accrual screen fires on Y1, is silent in Y2 -- and the reversal\n"
      "   looks like 'improving quality'.")

# --- mode 2: acquisition-inflated cash flow breaks the time series
print("\n--- mode 2: serial-acquirer distortion of the CFO/NI screen")
for label, cfo, ni in [("organic", 130.0, 105.0), ("acquired", 260.0, 105.0)]:
    print(f"   {label:9s} CFO={cfo:5.0f} NI={ni:5.0f}  CFO/NI={cfo/ni:5.2f}  "
          f"{'>>> looks pristine' if cfo/ni > 2.0 else 'normal'}")

# --- mode 3: cash-sales fabrication cancels out of balance-sheet accruals
print("\n--- mode 3: cash-sales manipulation is invisible to balance-sheet accruals")
print("   Sloan accruals = (dCA-dCash)-(dCL-dSTD-dTP)-Dep.")
print("   Book fake CASH revenue -> dCash rises with dCA -> accruals unperturbed.")
# show the cancellation explicitly
dCA, dCash = 100.0, 30.0
before = (dCA - dCash)
dCA2, dCash2 = dCA + 50.0, dCash + 50.0           # +50 of fabricated cash revenue
after = (dCA2 - dCash2)
print(f"   (dCA-dCash) before={before:.0f}  after a +50 cash-sale fabrication={after:.0f}  "
      f"-> unchanged: {abs(before-after) < 1e-9}")

# --- mode 4: binary composite signals reward improvement from a terrible base
print("\n--- mode 4: composite scores reward improvement from any base")
roa, p_roa = -0.02, -0.15
print(f"   ROA {p_roa:+.0%} -> {roa:+.0%}  dROA={roa-p_roa:+.0%}  "
      f"'improving' signal = {int(roa-p_roa>0)}  (firm is still loss-making)")
print("   A binary 'improving' signal cannot distinguish turnaround from terminal decline.")
```
```
--- mode 1: accrual reversal (stuffing today = negative accruals tomorrow)
   Y1 earnings = 100  (accruals +60, cash +40)
   Y2 earnings = -20  (accruals -60, cash +40)
   A naive accrual screen fires on Y1, is silent in Y2 -- and the reversal
   looks like 'improving quality'.

--- mode 2: serial-acquirer distortion of the CFO/NI screen
   organic   CFO=  130 NI=  105  CFO/NI= 1.24  normal
   acquired  CFO=  260 NI=  105  CFO/NI= 2.48  >>> looks pristine

--- mode 3: cash-sales manipulation is invisible to balance-sheet accruals
   Sloan accruals = (dCA-dCash)-(dCL-dSTD-dTP)-Dep.
   Book fake CASH revenue -> dCash rises with dCA -> accruals unperturbed.
   (dCA-dCash) before=70  after a +50 cash-sale fabrication=70  -> unchanged: True

--- mode 4: composite scores reward improvement from any base
   ROA -15% -> -2%  dROA=+13%  'improving' signal = 1  (firm is still loss-making)
   A binary 'improving' signal cannot distinguish turnaround from terminal decline.
```
