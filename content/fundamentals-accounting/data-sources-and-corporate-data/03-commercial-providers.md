...
```python
# Reconciling three commercial providers to ONE canonical EBITDA for FY2022.
# Each provider uses different field names and different adjustment choices.
from statistics import median

# provider-native field names -> canonical
field_map = {
    "sharadar":  {"ebitda": "EBITDA",     "revenue": "REVENUE"},
    "compustat": {"ebitda": "OIBDP",      "revenue": "SALE"},
    "yahoo":     {"ebitda": "ebitda",     "revenue": "totalRevenue"},
}
raw = {
    "sharadar":  {"EBITDA": 210.0, "REVENUE": 1000.0},
    "compustat": {"OIBDP":  218.0, "SALE":    1000.0},
    "yahoo":     {"ebitda": 206.0, "totalRevenue": 1000.0},
}
canon = {}
for prov, m in field_map.items():
    canon[prov] = {disp: raw[prov][native] for disp, native in m.items()}
    print(f"{prov:9s} {m} -> {canon[prov]}")

vals = [canon[p]["ebitda"] for p in canon]
print(f"\nEBITDA across providers : {vals}")
print(f"median = {median(vals):.1f}   spread = {max(vals)-min(vals):.1f} "
      f"({(max(vals)-min(vals))/median(vals)*100:.1f}% of median)")
print(f"median EV/EBITDA @ EV=2015 : {2015/median(vals):.2f}"
      f"   (range {2015/max(vals):.2f}-{2015/min(vals):.2f})")

# vendor disagreement is the rule, not the exception: document the adjustment
adj = {"sharadar": "as-reported GAAP", "compustat": "incl. other income (OIBDP)",
       "yahoo": "excl. some SBC"}
for p in canon:
    print(f"  {p:9s} EBITDA={canon[p]['ebitda']:.1f}  basis: {adj[p]}")
```
```
sharadar  {'ebitda': 'EBITDA', 'revenue': 'REVENUE'} -> {'ebitda': 210.0, 'revenue': 1000.0}
compustat {'ebitda': 'OIBDP', 'revenue': 'SALE'} -> {'ebitda': 218.0, 'revenue': 1000.0}
yahoo     {'ebitda': 'ebitda', 'revenue': 'totalRevenue'} -> {'ebitda': 206.0, 'revenue': 1000.0}

EBITDA across providers : [210.0, 218.0, 206.0]
median = 210.0   spread = 12.0 (5.7% of median)
median EV/EBITDA @ EV=2015 : 9.60   (range 9.24-9.78)
  sharadar  EBITDA=210.0  basis: as-reported GAAP
  compustat EBITDA=218.0  basis: incl. other income (OIBDP)
  yahoo     EBITDA=206.0  basis: excl. some SBC
```
