...
```python
# Liquidity & leverage for "Northstar Manufacturing" + Penman reformulation.
ca, cl = 230.0, 155.0; inv = 100.0
std, ltd = 50.0, 210.0; td = std + ltd
cash, equity = 45.0, 325.0
ebit, da = 170.0, 40.0; ebitda = ebit + da; interest = 20.0
# --- liquidity
cr, qr = ca/cl, (ca-inv)/cl
# --- leverage / coverage
de = td/equity
net_debt = td - cash
nd_ebitda = net_debt/ebitda
icov = ebit/interest
# --- Penman reformulation (Ch 9): separate financing debt from operating liab.
op_liab = 80.0 + 25.0                    # AP + accrued (non-debt operating liab.)
noa = (690.0 - 45.0) - op_liab           # operating assets - operating liabilities
nfo = td - cash                          # net financial obligations = 215
flev = nfo/equity
oll = op_liab/noa                        # operating liability leverage
print(f"Liquidity:  Current={cr:.3f}  Quick={qr:.3f}")
print(f"Leverage:   D/E={de:.3f}  NetDebt/EBITDA={nd_ebitda:.3f}  "
      f"IntCoverage={icov:.2f}x")
print(f"Penman:     FLEV(NFO/CSE)={flev:.3f}   OLLEV(OpLiab/NOA)={oll:.3f}")
print(f"Reformulated view: of {td:.0f} of liabilities, {op_liab:.0f} are operating "
      f"(non-debt); true financing obligations = {nfo:.0f}")
```
```
Liquidity:  Current=1.484  Quick=0.839
Leverage:   D/E=0.800  NetDebt/EBITDA=1.024  IntCoverage=8.50x
Penman:     FLEV(NFO/CSE)=0.662   OLLEV(OpLiab/NOA)=0.194
Reformulated view: of 260 of liabilities, 105 are operating (non-debt); true financing obligations = 215
```
