...
random.seed(7)
# --- Data mining: 200 random factors in a NULL world (no real alpha) ---
N_TRIALS, N_STOCKS, N_PERIODS = 200, 400, 60
returns = [[random.gauss(0,1) for _ in range(N_PERIODS)] for _ in range(N_STOCKS)]
def backtest_split(split):
    l=[i for i in range(N_STOCKS) if split[i]>0]
    s=[i for i in range(N_STOCKS) if split[i]<=0]
    per = [st.mean([returns[i][t] for i in l]) - st.mean([returns[i][t] for i in s])
           for t in range(N_PERIODS)]
    return st.mean(per)*12*100                     # annualized long-short return
best = -1e9; allres = []
for _ in range(N_TRIALS):
    split = [1 if random.random()<0.5 else -1 for _ in range(N_STOCKS)]
    r = backtest_split(split); allres.append(r); best = max(best, r)
print(f"mean across all {N_TRIALS} factors   = {st.mean(allres):+.2f}%/yr  (should be ~0)")
print(f"best single factor (data-mined)     = {best:+.2f}%/yr")
print(f"top-10 average                      = {st.mean(sorted(allres, reverse=True)[:10]):+.2f}%/yr")
# --- Look-ahead bias: value screen on point-in-time vs restated book ---
books_pit = [random.uniform(40,90) for _ in range(10)]     # as-reported book per share
books_rst = [b - random.uniform(0,25) for b in books_pit]  # restated (future write-downs)
mc = [random.uniform(60,120) for _ in range(10)]
bm_pit = [books_pit[i]/mc[i] for i in range(10)]
bm_rst = [books_rst[i]/mc[i] for i in range(10)]
def cheap_bm(bml): return st.mean(sorted(bml)[:3])         # avg B/M of the 3 cheapest
print(f"Cheapest-third avg B/M: point-in-time={cheap_bm(bm_pit):.3f}  "
      f"restated={cheap_bm(bm_rst):.3f}  <- restated looks cheaper")
```

```text
mean across all 200 factors   = +1.58%/yr  (should be ~0)
best single factor (data-mined)     = +39.93%/yr
top-10 average                      = +31.05%/yr
Cheapest-third avg B/M: point-in-time=0.465  restated=0.321  <- restated looks cheaper
```
