...
```python
# --- direct method: actual cash moved in operations ---
cash_from_customers = 5000 + 2000        # T5 cash sale + T8 collection
cash_to_suppliers   = 4000               # inventory bought for cash
cash_for_expenses   = 1500               # operating expenses paid
cfo_direct = cash_from_customers - cash_to_suppliers - cash_for_expenses

# --- indirect method: earnings + noncash + working-capital changes ---
ni, dep = 800, 1200
dAR, dInv, dAP = 2000 - 2000, 500, 0     # AR collected -> net 0; inventory 500
cfo_indirect = ni + dep - dAR - dInv + dAP

# --- accrual identity ---
accruals = (dAR + dInv) - dAP - dep
print(f"CFO (direct)   = {cfo_direct:,.0f}")
print(f"CFO (indirect) = {cfo_indirect:,.0f}")
print(f"Accruals       = {accruals:,.0f}   (dAR {dAR:+.0f} + dInv {dInv:+.0f} - dAP {dAP:+.0f} - Dep {(-dep):+.0f})")
print(f"Earnings = CFO + Accruals  ->  {ni:,.0f} = {cfo_indirect:,.0f} + ({accruals:,.0f})  {ni==cfo_indirect+accruals}")
```
```
CFO (direct)   = 1,500
CFO (indirect) = 1,500
Accruals       = -700   (dAR +0 + dInv +500 - dAP +0 - Dep -1200)
...
- Quality & factors: [[fundamentals-accounting/accounting-quality-and-red-flags/index|Accounting Quality & Red Flags]] · [[pillars/01-quantitative-research/fundamental-multi-factor-models|Fundamental Multi-Factor Models]] (Sloan accruals factor lives here)
