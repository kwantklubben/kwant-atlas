...
```python
import math, cmath
# ---- compact Heston pricer: Gatheral CF (2.12) + Lewis inversion (5.6), F = S0 e^{rT} ----
...
def heston_call(F,K,T,v0,vbar,lam,eta,rho,n=8000,U=400.0):
...
def implied_vol(p,F,K,T):
    lo,hi=1e-8,5.0
    for _ in range(200):
        m=0.5*(lo+hi)
        if bs_call(F,K,T,m)>p: hi=m
        else: lo=m
    return 0.5*(lo+hi)

# ---- Bergomi ch.6 one-factor (Heston) facts: (6.4), (6.20) and its limits ----
def vs_vol(T,V,vbar,k):                      # (6.4)
    x=k*T; return math.sqrt(vbar+(1.0-math.exp(-x))/x*(V-vbar))
def skew_flat(T,vbar,k,sigma,rho):           # (6.20)
    x=k*T; return rho*sigma/(2.0*math.sqrt(vbar))*(x+math.exp(-x)-1.0)/(x*x)
def I(x): return (1.0-math.exp(-x))/x

lam,eta,rho=1.3253,0.3877,-0.7165; v0,vbar=0.0174,0.0354; F=100.0

print("Heston one-factor facts (Bergomi ch.6), V0=0.0174, Vbar=0.0354, k=1.3253")
print(f"  Feller 2*k*Vbar > sigma^2 : {2*lam*vbar:.5f} > {eta*eta:.5f} ? {2*lam*vbar>eta*eta}")
print(f"  VS vol sigma_hat_T (6.4): short-T limit sqrt(V0) = {math.sqrt(v0)*100:.4f}%")
for T in (0.05,0.25,1.0,5.0,20.0):
    print(f"    T={T:6.2f}: sigma_hat_T = {vs_vol(T,v0,vbar,lam)*100:6.3f}%")
print("  ATMF skew S_T (6.20) vs its exact limits:")
print(f"    short-T limit rho*sigma/(4 sqrt(Vbar)) = {rho*eta/(4*math.sqrt(vbar)):+.6f}")
print(f"    long-T  limit rho*sigma/(2 sqrt(Vbar) kT):")
for T in (0.01,1.0,5.0,50.0):
    print(f"    T={T:6.2f}: S_T={skew_flat(T,vbar,lam,eta,rho):+.6f}   long-limit={rho*eta/(2*math.sqrt(vbar)*lam*T):+.6f}")

print("  independent check: implied-variance skew from the CF pricer vs rho*eta/2:")
print(f"    analytic rho*eta/2 = {rho*eta/2:+.6f}")
for T,h in ((0.002,2e-4),(0.005,2e-4),(0.005,1e-3),(0.010,5e-4)):
    c={}
    for s in (-1,0,1):
        K=F*math.exp(s*h)
        c[s]=implied_vol(heston_call(F,K,T,v0,vbar,lam,eta,rho),F,K,T)**2
    print(f"    T={T:6.4f} h={h:7.5f}: var-skew = {(c[1]-c[-1])/(2*h):+.6f}")

print("vol-of-vol term structure (shapes normalised at T=0.25): Heston (6.9) vs two-factor (7.39) vs benchmark (7.40)")
def nu_T(T,nu,th,k1,k2,r12):                 # (7.39)
    a1=I(k1*T); a2=I(k2*T); aw=1.0/math.sqrt((1-th)**2+th**2+2*r12*th*(1-th))
    return nu*aw*math.sqrt((1-th)**2*a1*a1+th**2*a2*a2+2*r12*th*(1-th)*a1*a2)
nu,th,k1,k2,r12=1.74,0.245,5.35,0.28,0.0        # Bergomi Set II
bm=lambda T:(0.25/T)**0.40
ref_hest,ref_tf,ref_bm=I(lam*0.25),nu_T(0.25,nu,th,k1,k2,r12),bm(0.25)
print(f"  {'T':>6}{'Heston I(kT)':>14}{'two-factor':>12}{'benchmark':>11}{'2f/bench':>10}{'Hest/bench':>12}")
for T in (0.10,0.25,0.50,1.00,2.00,5.00):
    h=I(lam*T)/ref_hest; t=nu_T(T,nu,th,k1,k2,r12)/ref_tf; b=bm(T)/ref_bm
    print(f"  {T:6.2f}{h:14.4f}{t:12.4f}{b:11.4f}{t/b:10.4f}{h/b:12.4f}")
```
```
Heston one-factor facts (Bergomi ch.6), V0=0.0174, Vbar=0.0354, k=1.3253
  Feller 2*k*Vbar > sigma^2 : 0.09383 > 0.15031 ? False
  VS vol sigma_hat_T (6.4): short-T limit sqrt(V0) = 13.1909%
    T=  0.05: sigma_hat_T = 13.410%
    T=  0.25: sigma_hat_T = 14.170%
    T=  1.00: sigma_hat_T = 15.946%
    T=  5.00: sigma_hat_T = 18.080%
    T= 20.00: sigma_hat_T = 18.634%
  ATMF skew S_T (6.20) vs its exact limits:
    short-T limit rho*sigma/(4 sqrt(Vbar)) = -0.369105
    long-T  limit rho*sigma/(2 sqrt(Vbar) kT):
    T=  0.01: S_T=-0.367480   long-limit=-55.701408
    T=  1.00: S_T=-0.248403   long-limit=-0.557014
    T=  5.00: S_T=-0.094613   long-limit=-0.111403
    T= 50.00: S_T=-0.010972   long-limit=-0.011140
  independent check: implied-variance skew from the CF pricer vs rho*eta/2:
    analytic rho*eta/2 = -0.138894
    T=0.0020 h=0.00020: var-skew = -0.120292
    T=0.0050 h=0.00020: var-skew = -0.138331
    T=0.0050 h=0.00100: var-skew = -0.138335
    T=0.0100 h=0.00050: var-skew = -0.138404
vol-of-vol term structure (shapes normalised at T=0.25): Heston (6.9) vs two-factor (7.39) vs benchmark (7.40)
       T  Heston I(kT)  two-factor  benchmark  2f/bench  Hest/bench
    0.10        1.1003      1.3212     1.4427    0.9158      0.7626
    0.25        1.0000      1.0000     1.0000    1.0000      1.0000
    0.50        0.8590      0.7274     0.7579    0.9598      1.1334
    1.00        0.6509      0.5340     0.5743    0.9298      1.1333
    2.00        0.4119      0.4186     0.4353    0.9616      0.9464
    5.00        0.1771      0.2816     0.3017    0.9333      0.5868
```
...
- Cross-links: [[pillars/03-derivative-pricing/volatility-surfaces-and-smiles/04-advanced-dynamics|VS · 04 Advanced Dynamics]] (SSR and the LV/Heston skew, from the surface side) · [[pillars/03-derivative-pricing/the-greeks-and-dynamic-hedging|The Greeks & Dynamic Hedging]] (cross-gamma/theta) · [[foundations/econometrics-and-time-series|Econometrics & Time Series]] (estimating $\nu_T$ and the SSR from data)
