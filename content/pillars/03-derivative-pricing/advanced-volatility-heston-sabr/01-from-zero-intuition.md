...
def implied_vol(price,F,K,T):                 # bisection: robust, always converges
    lo,hi=1e-8,5.0
    for _ in range(200):
        m=0.5*(lo+hi)
...
F,T,p=100.0,1.0,0.5                           # variance is 10%^2 w.p. 1/2, 30%^2 w.p. 1/2
s1,s2=0.10,0.30
print("Two-state random vol: sigma=10% w.p. 1/2, 30% w.p. 1/2")
print(f"mixture mean variance E[v]= {p*s1*s1+(1-p)*s2*s2:.5f} -> {math.sqrt(p*s1*s1+(1-p)*s2*s2)*100:.3f}% vol")
print(f"{'K':>6} {'price':>9} {'implied vol':>12}")
for K in (70.0,80.0,90.0,100.0,110.0,120.0,130.0):
    pr=p*bs_call(F,K,T,s1)+(1-p)*bs_call(F,K,T,s2)
    print(f"{K:6.0f} {pr:9.4f} {implied_vol(pr,F,K,T)*100:11.3f}%")
```
```
Two-state random vol: sigma=10% w.p. 1/2, 30% w.p. 1/2
mixture mean variance E[v]= 0.05000 -> 22.361% vol
     K     price  implied vol
    70   30.7150      24.967%
    80   21.7872      22.915%
    90   13.8626      20.830%
