...
# ---- (1) expected signature of 1D Brownian motion via seeded MC ----
random.seed(7)
T=1.0; M=200; dt=T/M; npaths=4000
acc=[0.0]*4
for _ in range(npaths):
    b=0.0
    for _ in range(M):
        b+=math.sqrt(dt)*random.gauss(0,1)
    acc[0]+=b; acc[1]+=b*b/2.0; acc[2]+=b**3/6.0; acc[3]+=b**4/24.0
print("(1) Expected signature of 1D Brownian motion, T=1 (seeded MC, %d paths):"%npaths)
print("    analytic: E[S^11]=T/2=0.5,  E[S^1111]=T^2/8=0.125,  odd levels = 0")
for m in range(4):
    an=[0.0,0.5,0.0,0.125][m]
    print("    E[S^%s] MC = %.5f   analytic = %.5f"%(
        '1'*(m+1) if m>0 else '1', acc[m]/npaths, an))

# ---- (2) signature kernel (truncated at level 2), Cauchy-Schwarz ----
...
def S_word(X,w):
    ...
    n=len(inc); tot=0.0
    for comp in compositions(len(w),n):
        prod=1.0; idx=0
        for k in range(n):
            c=comp[k]
            if c>0:
    ...
    for m in range(1,L+1):
        for i in range(d**m):
            ...
            for _ in range(m):
                w.append(q%d); q//=d
            words.append(list(w))
    return sum(S_word(X,w)*S_word(Y,w) for w in words)
P=[(0.0,0.0),(2.0,0.0),(2.0,3.0),(6.0,1.0),(3.0,2.0)]
Q=[(0.0,0.0),(1.0,1.0),(4.0,1.0),(4.0,4.0),(6.0,3.0)]
print("\n(2) Signature kernel (truncated to level 2):")
kpp,kqq,kpq=kernel(P,P),kernel(Q,Q),kernel(P,Q)
print("    K(X,X)=%.6f   K(Y,Y)=%.6f   K(X,Y)=%.6f"% (kpp,kqq,kpq))
print("    Cauchy-Schwarz K(X,Y)^2 <= K(X,X)K(Y,Y):  %.6f <= %.6f -> %s"%(
    kpq*kpq, kpp*kqq, "OK" if kpq*kpq<=kpp*kqq+1e-9 else "FAIL"))

# ---- (3) rough-vol lead-lag: a fractional (rough) path has diverging p-variation ----
def levy_area_2d(B1,B2,M):
    n=len(B1)-1; step=n//M
    i1=[B1[k] for k in range(0,n+1,step)]; i2=[B2[k] for k in range(0,n+1,step)]
    d1=[i1[k+1]-i1[k] for k in range(len(i1)-1)]; d2=[i2[k+1]-i2[k] for k in range(len(i2)-1)]
    m=len(d1); A=0.0
    for k in range(m):
        for l in range(k+1,m): A+=0.5*(d1[k]*d2[l]-d2[k]*d1[l])
    return A
random.seed(11)
Mbase=8000; dt=1.0/Mbase
B1=[0.0]; B2=[0.0]
for _ in range(Mbase):
    B1.append(B1[-1]+math.sqrt(dt)*random.gauss(0,1))
    B2.append(B2[-1]+math.sqrt(dt)*random.gauss(0,1))
print("\n(3) lead-lag Levy area of a Brownian 2D path (rough-vol building block):")
for M in (1000,2000,4000,8000):
    print("    grid %5d: Levy area = %+.5f"% (M, levy_area_2d(B1,B2,M)))
```
```
(1) Expected signature of 1D Brownian motion, T=1 (seeded MC, 4000 paths):
    analytic: E[S^11]=T/2=0.5,  E[S^1111]=T^2/8=0.125,  odd levels = 0
    E[S^1] MC = 0.00171   analytic = 0.00000
    E[S^11] MC = 0.49055   analytic = 0.50000
    E[S^111] MC = 0.00383   analytic = 0.00000
    E[S^1111] MC = 0.11719   analytic = 0.12500

(2) Signature kernel (truncated to level 2):
    K(X,X)=56.750000   K(Y,Y)=556.750000   K(X,Y)=170.500000
    Cauchy-Schwarz K(X,Y)^2 <= K(X,X)K(Y,Y):  29070.250000 <= 31595.562500 -> OK

(3) lead-lag Levy area of a Brownian 2D path (rough-vol building block):
    grid  1000: Levy area = +0.24436
    grid  2000: Levy area = +0.24764
    grid  4000: Levy area = +0.23906
    grid  8000: Levy area = +0.23318
```
