...
def bsm_put(S, X, T, r, sig):
    d1 = (math.log(S/X) + (r + 0.5*sig**2)*T) / (sig*math.sqrt(T))
    d2 = d1 - sig*math.sqrt(T)
    return X*math.exp(-r*T)*N(-d2) - S*N(-d1)

def mc_vanilla_vs_lookback(S, X, T, r, sig, nsteps, npath, seed):
    """Simulate under Q (drift = r). Return (vanilla put price, lookback put price)
       on the SAME paths: a lookback put pays X - min(S over path)."""
    rnd = random.Random(seed); put = 0.0; lk = 0.0; dt = T/nsteps
    for _ in range(npath):
        St = S; mn = S
        for _ in range(nsteps):
            St *= math.exp((r - 0.5*sig**2)*dt + sig*math.sqrt(dt)*rnd.gauss(0,1))
            mn = min(mn, St)
        put += max(X - St, 0.0)          # vanilla: terminal only
        lk  += max(X - mn, 0.0)          # lookback: uses the running min
    disc = math.exp(-r*T)
    return disc*put/npath, disc*lk/npath

S, X, T, r, sig = 100.0, 100.0, 0.5, 0.05, 0.20
vp, lp = mc_vanilla_vs_lookback(S, X, T, r, sig, 100, 300000, 7)
print(f"vanilla  put (terminal only)   = {vp:.4f}")
print(f"lookback put (path-dependent)  = {lp:.4f}")
print(f"closed-form BSM put            = {bsm_put(S,X,T,r,sig):.4f}")
```
```
