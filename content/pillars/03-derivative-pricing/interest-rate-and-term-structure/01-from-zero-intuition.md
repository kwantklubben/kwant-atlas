...
def mc_zcb(short_rate_step, r0, T, steps, n, disc_fun):
    """Bond = E^Q[exp(-∫_0^T r ds)] by MC of the short-rate path."""
    dt=T/steps; tot=0.0
    for _ in range(n):
        r=r0; acc=0.0
        for _ in range(steps):
            r=short_rate_step(r, dt)          # one model step under Q
            acc+=r*dt
        tot+=disc_fun(acc)
    return tot/n

# Vasicek step under Q:  dr = a(b-r) dt + sigma dW
def vas_step(a,b,sig):
    return lambda r,dt: r + a*(b-r)*dt + sig*math.sqrt(dt)*random.gauss(0,1)

random.seed(1)
P = mc_zcb(vas_step(0.15,0.05,0.01), 0.04, 5.0, 200, 200000, lambda a: math.exp(-a))
print(f"Vasicek MC bond P(0,5) = {P:.6f}   (expect ~0.8077, the closed form)")
```
```
Vasicek MC bond P(0,5) = 0.807521   (expect ~0.8077, the closed form)
```
