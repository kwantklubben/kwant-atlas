...
a1,a2,p=0.10,0.30,0.5
a1s,a2s=a1*a1,a2*a2
def rho_star(t,y): return p*gauss(y,a1s*t)+(1-p)*gauss(y,a2s*t)
def sigD2(t,y):    return (p*a1s*gauss(y,a1s*t)+(1-p)*a2s*gauss(y,a2s*t))/rho_star(t,y)
print("Market: return X_t = alpha*W_t with alpha in {0.10,0.30} each w.p. 1/2.")
print("  E[alpha]=%.6f  E[alpha^2]=%.6f  E[1/alpha]=%.6f"%(p*a1+(1-p)*a2,p*a1s+(1-p)*a2s,p/a1+(1-p)/a2))
print("  lim_t->0 sigma_D(t,0) = sqrt(E[alpha]/E[1/alpha]) = %.6f -> %.4f%% implied vol"%(
      math.sqrt((p*a1+(1-p)*a2)/(p/a1+(1-p)/a2)),100*math.sqrt((p*a1+(1-p)*a2)/(p/a1+(1-p)/a2))))
print()
print("(A) Gyongy's Markovian-projection identity,  d_t rho* = (1/2) d_yy ( sigma_D^2 * rho* ):")
h=1e-3
for t in (0.25,0.50):
    for y in (-0.20,0.00,0.20):
        f=lambda yy: sigD2(t,yy)*rho_star(t,yy)
        lhs=(rho_star(t+h,y)-rho_star(t-h,y))/(2*h); rhs=0.5*(f(y+h)-2*f(y)+f(y-h))/h**2
        print("      t=%.2f y=%+.2f :  d_t rho*=%.9f   1/2 d_yy(sigma_D^2 rho*)=%.9f   rel.err=%.1e"
              %(t,y,lhs,rhs,abs(lhs-rhs)/abs(lhs)))
print()
print("(B) Local variance averages to the expected instantaneous variance under the market law:")
n=4000; lo,hi=-1.0,1.0; hh=(hi-lo)/n; s=0.0
for i in range(n+1):
    y=lo+i*hh; s+=(1.0 if i in (0,n) else (4.0 if i%2 else 2.0))*sigD2(0.5,y)*rho_star(0.5,y)
print("      integral of sigma_D^2(t,y)*rho*(t,y) dy at t=0.5 = %.6f   vs E[alpha^2] = %.6f"%(s*hh/3.0,p*a1s+(1-p)*a2s))
print()
print("(C) The projection of the LSV diffusion sigma(t,y)*alpha has local variance sigma(t,y)^2*m(t,y);")
print("    m(t,y)=E[alpha^2|X_t=y].  The leverage choice sigma^2 = sigma_D^2/m makes it equal sigma_D^2.")
m=lambda t,y:(p*a1s*gauss(y,a1s*t)+(1-p)*a2s*gauss(y,a2s*t))/(p*gauss(y,a1s*t)+(1-p)*gauss(y,a2s*t))
print("      t=0.5: m(y=-0.2)=%.6f  m(0)=%.6f  m(+0.2)=%.6f   (E[alpha^2]=%.6f)"%(m(0.5,-0.2),m(0.5,0),m(0.5,0.2),p*a1s+(1-p)*a2s))
print("      => leverage sigma(0.5,y) = sqrt(sigma_D^2/m) = %.6f / %.6f / %.6f  (== 1: the market IS this driver)"%(
      math.sqrt(sigD2(0.5,-0.2)/m(0.5,-0.2)),math.sqrt(sigD2(0.5,0)/m(0.5,0)),math.sqrt(sigD2(0.5,0.2)/m(0.5,0.2))))
print("    The CONDITIONAL variance is far from the unconditional one:")
print("      m(0.5,+/-0.2)=%.6f  m(0.5,0)=%.6f  E[alpha^2]=%.6f  -> +%.0f%% / -%.0f%% at the wings"
      %(m(0.5,0.2),m(0.5,0),p*a1s+(1-p)*a2s,100*(m(0.5,0.2)/(p*a1s+(1-p)*a2s)-1),100*(1-m(0.5,0)/(p*a1s+(1-p)*a2s))))
print("      substituting the unconditional mean for E[v_t|S_t] would mis-state the local variance by that much.")
```
```
Market: return X_t = alpha*W_t with alpha in {0.10,0.30} each w.p. 1/2.
  E[alpha]=0.200000  E[alpha^2]=0.050000  E[1/alpha]=6.666667
  lim_t->0 sigma_D(t,0) = sqrt(E[alpha]/E[1/alpha]) = 0.173205 -> 17.3205% implied vol

(A) Gyongy's Markovian-projection identity,  d_t rho* = (1/2) d_yy ( sigma_D^2 * rho* ):
      t=0.25 y=-0.20 :  d_t rho*=0.890590312   1/2 d_yy(sigma_D^2 rho*)=0.890567973   rel.err=2.5e-05
      t=0.25 y=+0.00 :  d_t rho*=-10.638567197   1/2 d_yy(sigma_D^2 rho*)=-10.637633428   rel.err=8.8e-05
      t=0.25 y=+0.20 :  d_t rho*=0.890590312   1/2 d_yy(sigma_D^2 rho*)=0.890567973   rel.err=2.5e-05
      t=0.50 y=-0.20 :  d_t rho*=0.294682508   1/2 d_yy(sigma_D^2 rho*)=0.294696641   rel.err=4.8e-05
      t=0.50 y=+0.00 :  d_t rho*=-3.761273294   1/2 d_yy(sigma_D^2 rho*)=-3.761117624   rel.err=4.1e-05
      t=0.50 y=+0.20 :  d_t rho*=0.294682508   1/2 d_yy(sigma_D^2 rho*)=0.294696641   rel.err=4.8e-05

(B) Local variance averages to the expected instantaneous variance under the market law:
      integral of sigma_D^2(t,y)*rho*(t,y) dy at t=0.5 = 0.050000   vs E[alpha^2] = 0.050000

(C) The projection of the LSV diffusion sigma(t,y)*alpha has local variance sigma(t,y)^2*m(t,y);
    m(t,y)=E[alpha^2|X_t=y].  The leverage choice sigma^2 = sigma_D^2/m makes it equal sigma_D^2.
      t=0.5: m(y=-0.2)=0.083685  m(0)=0.030000  m(+0.2)=0.083685   (E[alpha^2]=0.050000)
      => leverage sigma(0.5,y) = sqrt(sigma_D^2/m) = 1.000000 / 1.000000 / 1.000000  (== 1: the market IS this driver)
    The CONDITIONAL variance is far from the unconditional one:
      m(0.5,+/-0.2)=0.083685  m(0.5,0)=0.030000  E[alpha^2]=0.050000  -> +67% / -40% at the wings
      substituting the unconditional mean for E[v_t|S_t] would mis-state the local variance by that much.
```
