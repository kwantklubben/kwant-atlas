...
def merton_call(F,K,T,sd,lamJ,muJ,dJ,nmax=60):
    kap=math.exp(muJ+0.5*dJ*dJ)-1.0; tot=0.0
    for n in range(nmax+1):
        wt=math.exp(-lamJ*T)*(lamJ*T)**n/math.factorial(n)
        tot+=wt*bs_call(F*math.exp(-lamJ*kap*T+n*muJ+0.5*n*dJ*dJ),K,T,math.sqrt(sd*sd+n*dJ*dJ/T))
    return tot
F=100.0
def dup(K,T,pars,hT=1e-4):
    hK=1e-3*K
    H=lambda KK,TT: merton_call(F,KK,TT,*pars)
    dT=(H(K,T+hT)-H(K,T-hT))/(2*hT); dKK=(H(K+hK,T)-2*H(K,T)+H(K-hK,T))/hK**2
    return dT/(0.5*K*K*dKK)
print("(a) CONTROL - no jumps (lambda=0): Dupire's formula returns sigma_diff^2 = 0.022500 exactly")
for T in (0.01,0.10,1.00):
    print("      T=%5.2f : %.9f"%(T,dup(100.0,T,(0.15,0.0,-0.1,0.15))))
print()
print("(b) WITH jumps (lambda=0.5, mu=-0.10, delta=0.15): the instantaneous quadratic-variation rate is")
print("      sigma_diff^2 + lambda*E[(e^J-1)^2] = 0.022500 + 0.013133 = %.6f (constant in t)"%(0.0225+0.5*(math.exp(-0.2+0.045)-2*math.exp(-0.1+0.01125)+1)))
print("      but Dupire's local variance is NOT that constant:")
print("        T       Dupire local var (ATM)     - QV rate")
for T in (0.005,0.02,0.10,1.00):
    d=dup(100.0,T,(0.15,0.5,-0.10,0.15)); q=0.0225+0.5*(math.exp(-0.2+0.045)-2*math.exp(-0.1+0.01125)+1)
    print("      %6.3f        %12.6f          %+12.6f"%(T,d,d-q))
print("      -> Dupire local variance is a MIXTURE of the diffusive variance and the jump effect; it")
print("         converges to the QV rate only at long maturity.  Treating sigma_loc^2 = sigma_diff^2 + QV_jump")
print("         as an identity (to 'strip the jump part') is a first-principles error.")
print()
print("(c) the two exact limits of the leverage")
print("      target = the SV driver's OWN smile -> leverage == 1  (Heston: 1.00197 at t=0, Dupire loc/sqrt(v0))")
print("      deterministic driver (vol-of-vol -> 0): m=xi_0^t known -> sigma=sigma_loc/sqrt(xi_0^t), i.e. LV:")
for t in (0.0,0.5,1.0,5.0):
    xi=0.0354+(0.0174-0.0354)*math.exp(-1.3253*t)
    print("        t=%4.1f: xi_0^t=%.6f  sigma(t)=0.20/sqrt(xi_0^t)=%.6f  local variance sigma^2*xi=%.6f"%(t,xi,0.20/math.sqrt(xi),0.20*0.20))
print()
print("(d) the 'usable LSV' gauge: zeta^u -> phi^u zeta^u with sigma(u,S) -> sigma(u,S)/sqrt(phi^u)")
for phit,phiu,sig,zeta in ((2.0,2.0,0.35,0.04),(0.25,0.25,0.35,0.04),(4.0,1.0,0.35,0.04)):
    print("      phi^t=%.2f phi^u=%.2f : sigma^2 zeta^t = %.9f -> %.9f   invariant=%s"
          %(phit,phiu,sig*sig*zeta,(sig/math.sqrt(phiu))**2*(phiu*zeta),abs(sig*sig*zeta-(sig/math.sqrt(phiu))**2*(phiu*zeta))<1e-15))
print("      consequence: the leverage is defined only up to this gauge; fixing it is a modelling choice.")
print()
print("(e) degrees of freedom: the leverage is DETERMINED by the SV driver, not fitted to vanillas")
print("      vanillas constrain only the product: sigma(t,S)^2 * m(t,S) = sigma_loc^2(t,S)")
print("      leverage grid 20x10 = 200 numbers | 5 strikes x 7 maturities = 35 vanilla quotes")
print("      with a 2- or 3-factor driftless forward-variance driver the leverage stays a function of (t,S) only")
print("      -> 2-dimensional regardless of the number of vol factors; the extra factors change m, not the")
print("         domain of the calibration, and they raise Var(v|S) and hence the estimation noise of m.")
v0,vbar,lam,eta=0.0174,0.0354,1.3253,0.3877
VarCIR=v0*eta*eta*(math.exp(-lam)-math.exp(-2*lam))/lam+vbar*eta*eta*(1-math.exp(-lam))**2/(2*lam)
print("      CIR: Var(v_1)=%.6f (sd %.6f) -> 1-factor binning SE(m)=sqrt(Var(v|S)/n_bin); more factors raise it."%(VarCIR,math.sqrt(VarCIR)))
```
```
(a) CONTROL - no jumps (lambda=0): Dupire's formula returns sigma_diff^2 = 0.022500 exactly
      T= 0.01 : 0.022508607
      T= 0.10 : 0.022500829
      T= 1.00 : 0.022500076

(b) WITH jumps (lambda=0.5, mu=-0.10, delta=0.15): the instantaneous quadratic-variation rate is
      sigma_diff^2 + lambda*E[(e^J-1)^2] = 0.022500 + 0.013133 = 0.035633 (constant in t)
      but Dupire's local variance is NOT that constant:
        T       Dupire local var (ATM)     - QV rate
       0.005            0.024213             -0.011420
       0.020            0.025732             -0.009901
       0.100            0.028913             -0.006721
       1.000            0.035077             -0.000556
      -> Dupire local variance is a MIXTURE of the diffusive variance and the jump effect; it
         converges to the QV rate only at long maturity.  Treating sigma_loc^2 = sigma_diff^2 + QV_jump
         as an identity (to 'strip the jump part') is a first-principles error.

(c) the two exact limits of the leverage
      target = the SV driver's OWN smile -> leverage == 1  (Heston: 1.00197 at t=0, Dupire loc/sqrt(v0))
      deterministic driver (vol-of-vol -> 0): m=xi_0^t known -> sigma=sigma_loc/sqrt(xi_0^t), i.e. LV:
        t= 0.0: xi_0^t=0.017400  sigma(t)=0.20/sqrt(xi_0^t)=1.516196  local variance sigma^2*xi=0.040000
        t= 0.5: xi_0^t=0.026121  sigma(t)=0.20/sqrt(xi_0^t)=1.237464  local variance sigma^2*xi=0.040000
        t= 1.0: xi_0^t=0.030617  sigma(t)=0.20/sqrt(xi_0^t)=1.143007  local variance sigma^2*xi=0.040000
        t= 5.0: xi_0^t=0.035376  sigma(t)=0.20/sqrt(xi_0^t)=1.063346  local variance sigma^2*xi=0.040000

(d) the 'usable LSV' gauge: zeta^u -> phi^u zeta^u with sigma(u,S) -> sigma(u,S)/sqrt(phi^u)
      phi^t=2.00 phi^u=2.00 : sigma^2 zeta^t = 0.004900000 -> 0.004900000   invariant=True
      phi^t=0.25 phi^u=0.25 : sigma^2 zeta^t = 0.004900000 -> 0.004900000   invariant=True
      phi^t=4.00 phi^u=1.00 : sigma^2 zeta^t = 0.004900000 -> 0.004900000   invariant=True
      consequence: the leverage is defined only up to this gauge; fixing it is a modelling choice.

(e) degrees of freedom: the leverage is DETERMINED by the SV driver, not fitted to vanillas
      vanillas constrain only the product: sigma(t,S)^2 * m(t,S) = sigma_loc^2(t,S)
      leverage grid 20x10 = 200 numbers | 5 strikes x 7 maturities = 35 vanilla quotes
      with a 2- or 3-factor driftless forward-variance driver the leverage stays a function of (t,S) only
      -> 2-dimensional regardless of the number of vol factors; the extra factors change m, not the
         domain of the calibration, and they raise Var(v|S) and hence the estimation noise of m.
      CIR: Var(v_1)=0.001467 (sd 0.038307) -> 1-factor binning SE(m)=sqrt(Var(v|S)/n_bin); more factors raise it.
```
