...
def gauss(x,s2t): return math.exp(-0.5*x*x/s2t)/math.sqrt(s2t)/SQ2PI
a1,a2,p=0.10,0.30,0.5; a1s,a2s=a1*a1,a2*a2; Ea2=p*a1s+(1-p)*a2s
print("================ (1) the forward-variance proxy cannot stand in for E[v_t|S_t] ================")
print("Two-state toy (alpha in {0.10,0.30} w.p. 1/2): E[alpha^2]=%.6f  -> the 'proxy' a desk would use"%Ea2)
print()
print("   t     y=ln S/F    m(t,y)=E[a^2|X=y]   proxy=E[a^2]   m/proxy-1   leverage err if proxy used")
for t in (0.25,1.0):
    for y in (-0.40,0.0,0.40):
        num=p*a1s*gauss(y,a1s*t)+(1-p)*a2s*gauss(y,a2s*t); den=p*gauss(y,a1s*t)+(1-p)*gauss(y,a2s*t)
        m=num/den
        print("  %.2f   %+.2f        %14.6f   %11.6f   %+9.2f%%   %+9.2f%%"%(t,y,m,Ea2,100*(m/Ea2-1),100*(math.sqrt(Ea2/m)-1)))
print("  Using the unconditional forward variance for m makes the model's local variance")
print("  sigma_loc^2 * (m/proxy) and the leverage wrong by the square root of the same ratio ->")
print("  the smile that comes out is the WRONG one (see the particle-method run in 03).")
print()
print("================ (2) error propagation:  d ln(sigma) = 0.5*( d ln sigma_loc - d ln m ) ========")
print("   relative error in m      ->  relative error in leverage   ->  relative error in local variance")
for e in (-0.20,-0.10,0.0,0.10,0.20):
    print("        %+6.1f%%                %+8.2f%%                      %+8.2f%%"%(100*e,-100*0.5*e,100*e))
print()
print("================ (3) the 'usable LSV' gauge:  zeta^u -> phi^u zeta^u , sigma(u,S) -> sigma/ sqrt(phi^u)")
print("   The instantaneous variance sigma(t,S)^2 zeta^t is invariant, so the S-process is unchanged:")
for phi_t,phi_u in ((2.0,2.0),(0.5,0.5),(3.0,1.0)):
    zeta=0.04; sig=0.35
    before=sig*sig*zeta
    after=(sig/math.sqrt(phi_u))**2*(phi_u*zeta)
    print("   phi^t=%.1f phi^u=%.1f : sigma^2 zeta  = %.8f -> %.8f   (invariant: %s)"%(phi_t,phi_u,before,after,abs(before-after)<1e-15))
print()
print("================ (4) binning noise of the conditional-variance estimate =======================")
...
print("  CIR variance at t=1, analytic: Var(v_1)=%.6f  (sd=%.6f)"%(VarCIR,math.sqrt(VarCIR)))
# independent Monte-Carlo check of that formula
random.seed(7); NP,NS=20000,500; dt=1.0/NS; acc=0.0; acc2=0.0
for _ in range(NP):
    v=v0
    for _ in range(NS):
        vp=v if v>0 else 0.0
        v=(math.sqrt(vp)+0.5*eta*math.sqrt(dt)*random.gauss(0,1))**2-lam*(vp-vbar)*dt-0.25*eta*eta*dt
    acc+=v; acc2+=v*v
mean=acc/NP; var=acc2/NP-mean*mean
print("  CIR variance at t=1, Monte Carlo (%d paths x %d Milstein steps): %.6f  (sd=%.6f)"%(NP,NS,var,math.sqrt(var)))
print()
print("  standard error of m at the money (m=0.0303): SE(m)=sqrt(Var(v|S)/n_bin)")
for nb in (500,2000,5000,10000):
    se=math.sqrt(VarCIR/nb)
    print("    paths per bin %6d -> SE(m)=%.6f  (%.1f%% of m)  -> leverage error %.1f%%  -> local-vol error %.2f vol pts"
          %(nb,se,100*se/0.0303,50*se/0.0303,100*math.sqrt(0.04)*(0.5*se/0.0303)))
```
```
================ (1) the forward-variance proxy cannot stand in for E[v_t|S_t] ================
Two-state toy (alpha in {0.10,0.30} w.p. 1/2): E[alpha^2]=0.050000  -> the 'proxy' a desk would use

   t     y=ln S/F    m(t,y)=E[a^2|X=y]   proxy=E[a^2]   m/proxy-1   leverage err if proxy used
  0.25   -0.40              0.090000      0.050000      +80.00%      -25.46%
  0.25   +0.00              0.030000      0.050000      -40.00%      +29.10%
  0.25   +0.40              0.090000      0.050000      +80.00%      -25.46%
  1.00   -0.40              0.089805      0.050000      +79.61%      -25.38%
  1.00   +0.00              0.030000      0.050000      -40.00%      +29.10%
  1.00   +0.40              0.089805      0.050000      +79.61%      -25.38%
  Using the unconditional forward variance for m makes the model's local variance
  sigma_loc^2 * (m/proxy) and the leverage wrong by the square root of the same ratio ->
  the smile that comes out is the WRONG one (see the particle-method run in 03).

================ (2) error propagation:  d ln(sigma) = 0.5*( d ln sigma_loc - d ln m ) ========
   relative error in m      ->  relative error in leverage   ->  relative error in local variance
         -20.0%                  +10.00%                        -20.00%
         -10.0%                   +5.00%                        -10.00%
          +0.0%                   -0.00%                         +0.00%
         +10.0%                   -5.00%                        +10.00%
...
================ (3) the 'usable LSV' gauge:  zeta^u -> phi^u zeta^u , sigma(u,S) -> sigma/ sqrt(phi^u)
   The instantaneous variance sigma(t,S)^2 zeta^t is invariant, so the S-process is unchanged:
   phi^t=2.0 phi^u=2.0 : sigma^2 zeta  = 0.00490000 -> 0.00490000   (invariant: True)
   phi^t=0.5 phi^u=0.5 : sigma^2 zeta  = 0.00490000 -> 0.00490000   (invariant: True)
   phi^t=3.0 phi^u=1.0 : sigma^2 zeta  = 0.00490000 -> 0.00490000   (invariant: True)

================ (4) binning noise of the conditional-variance estimate =======================
  CIR variance at t=1, analytic: Var(v_1)=0.001467  (sd=0.038307)
  CIR variance at t=1, Monte Carlo (20000 paths x 500 Milstein steps): 0.001471  (sd=0.038349)

  standard error of m at the money (m=0.0303): SE(m)=sqrt(Var(v|S)/n_bin)
    paths per bin    500 -> SE(m)=0.001713  (5.7% of m)  -> leverage error 2.8%  -> local-vol error 0.57 vol pts
    paths per bin   2000 -> SE(m)=0.000857  (2.8% of m)  -> leverage error 1.4%  -> local-vol error 0.28 vol pts
    paths per bin   5000 -> SE(m)=0.000542  (1.8% of m)  -> leverage error 0.9%  -> local-vol error 0.18 vol pts
    paths per bin  10000 -> SE(m)=0.000383  (1.3% of m)  -> leverage error 0.6%  -> local-vol error 0.13 vol pts
```
