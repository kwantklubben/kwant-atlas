...
def baw(S, X, T, r, b, sig, kind):
    c, p, d1 = bsm(S, X, T, r, b, sig)
    if kind == 'call' and b >= r: return c
    M = 2*r/sig**2; Nn = 2*b/sig**2; Kk = 1 - math.exp(-r*T)
    if kind == 'call':
        q2 = (-(Nn-1) + math.sqrt((Nn-1)**2 + 4*M/Kk))/2
        g  = lambda s: (s-X) - (bsm(s,X,T,r,b,sig)[0] + (1-math.exp((b-r)*T)*N(bsm(s,X,T,r,b,sig)[2]))*s/q2)
        lo, hi = X, 1e4*X
        for _ in range(200):
            m = 0.5*(lo+hi)
            if g(m) > 0: hi = m
            else:        lo = m
        Ss = 0.5*(lo+hi); A2 = (1-math.exp((b-r)*T)*N(bsm(Ss,X,T,r,b,sig)[2]))*Ss/q2
        return (S-X) if S >= Ss else c + A2*(S/Ss)**q2
    else:
        q1 = (-(Nn-1) - math.sqrt((Nn-1)**2 + 4*M/Kk))/2
        g  = lambda s: (X-s) - (bsm(s,X,T,r,b,sig)[1] - (1-math.exp((b-r)*T)*N(-bsm(s,X,T,r,b,sig)[2]))*s/q1)
        lo, hi = 1e-8, X
        for _ in range(200):
            m = 0.5*(lo+hi)
            if g(m) > 0: lo = m
...
def bs93_call(S, X, T, r, b, sig):              # Bjerksund-Stensland 1993 (Haug 3.2)
    if b >= r: return bsm(S,X,T,r,b,sig)[0]
    beta = (0.5 - b/sig**2) + math.sqrt((b/sig**2 - 0.5)**2 + 2*r/sig**2)
    Binf = beta*X/(beta-1.0); B0 = max(X, r/(r-b)*X)
    hT   = -(b*T + 2*sig*math.sqrt(T))*B0/(Binf-B0)
    I    = B0 + (Binf-B0)*(1-math.exp(hT))
    if S >= I: return S - X
    def phi(S,T,gam,H,I):
        lam = (-r + gam*b + 0.5*gam*(gam-1)*sig**2)*T
        d   = -(math.log(S/H) + (b + (gam-0.5)*sig**2)*T)/(sig*math.sqrt(T))
        kap = 2*b/sig**2 + (2*gam-1)
        return math.exp(lam)*S**gam*(N(d) - (I/S)**kap*N(d - 2*math.log(I/S)/(sig*math.sqrt(T))))
    alpha = (I-X)*I**(-beta)
    return (alpha*S**beta - alpha*phi(S,T,beta,I,I) + phi(S,T,1,I,I) - phi(S,T,1,X,I)
            - X*phi(S,T,0,I,I) + X*phi(S,T,0,X,I))

def bs93_put(S, X, T, r, b, sig):
    return bs93_call(X, S, T, r-b, -b, sig)      # put-call transformation (Haug 3.4)

def crr_put(S, X, T, r, b, sig, n):
    dt = T/n; u = math.exp(sig*math.sqrt(dt)); d = 1.0/u; p = (math.exp(b*dt)-d)/(u-d)
...
print("BAW American call, X=100 r=10% b=0 (Haug Table 3-1):")
print(f"  S=100 T=0.10 sig=0.15 -> {baw(100,100,0.10,0.10,0.0,0.15,'call'):.4f}  (Haug 1.8771)")
print(f"  S=110 T=0.50 sig=0.35 -> {baw(110,100,0.50,0.10,0.0,0.35,'call'):.4f}  (Haug 15.5689)")
print("Bjerksund-Stensland 1993 (Haug): S=42 X=40 T=0.75 r=4% b=-4% sig=35%")
print(f"  BS93 call = {bs93_call(42,40,0.75,0.04,-0.04,0.35):.4f}  (Haug 5.2704; European {bsm(42,40,0.75,0.04,-0.04,0.35)[0]:.4f})")
print("BAW vs CRR American put, X=95 r=b=8% sig=30%:")
for S0 in (90.0, 100.0, 110.0):
    a = baw(S0,95.0,0.5,0.08,0.08,0.30,'put'); t = crr_put(S0,95.0,0.5,0.08,0.08,0.30,4000)
    print(f"  S={S0:5.0f}: BAW={a:.4f}  CRR={t:.4f}  error={a-t:+.4f}")
```
```
BAW American call, X=100 r=10% b=0 (Haug Table 3-1):
  S=100 T=0.10 sig=0.15 -> 1.8769  (Haug 1.8771)
  S=110 T=0.50 sig=0.35 -> 15.5684  (Haug 15.5689)
Bjerksund-Stensland 1993 (Haug): S=42 X=40 T=0.75 r=4% b=-4% sig=35%
  BS93 call = 5.2704  (Haug 5.2704; European 5.0975)
BAW vs CRR American put, X=95 r=b=8% sig=30%:
  S=   90: BAW=8.8692  CRR=8.8989  error=-0.0298
  S=  100: BAW=4.7129  CRR=4.6918  error=+0.0211
  S=  110: BAW=2.3415  CRR=2.3041  error=+0.0373
```
