...
def simulate(S0, r, sig, T, m, n, seed):
    random.seed(seed); dt = T/m
    P = [[S0]*(m+1) for _ in range(n)]
    for i in range(n):
        s = S0
        for k in range(1, m+1):
            s *= math.exp((r-0.5*sig**2)*dt + sig*math.sqrt(dt)*random.gauss(0,1)); P[i][k] = s
    return P

def solve(A, b):
    n = len(b); M = [A[i][:]+[b[i]] for i in range(n)]
    for c in range(n):
        pv = max(range(c,n), key=lambda r: abs(M[r][c])); M[c], M[pv] = M[pv], M[c]
        for r in range(n):
            if r != c:
                f = M[r][c]/M[c][c]
                for cc in range(c, n+1): M[r][cc] -= f*M[c][cc]
    return [M[i][n]/M[i][i] for i in range(n)]

def lsm_fit(paths, X, r, T, m, deg=2):
    dt = T/m; disc = math.exp(-r*dt); n = len(paths)
    V = [[0.0]*(m+1) for _ in range(n)]
    for i in range(n): V[i][m] = max(X - paths[i][m], 0.0)
    coefs = [None]*(m+1)
    for k in range(m-1, 0, -1):
        itm = [i for i in range(n) if X - paths[i][k] > 0.0]
        if len(itm) >= deg+1:
            A = [[sum(paths[i][k]**(a+b) for i in itm) for b in range(deg+1)] for a in range(deg+1)]
            y = [sum(paths[i][k]**a * V[i][k+1]*disc for i in itm) for a in range(deg+1)]
            coefs[k] = solve(A, y)
        for i in range(n):
            cont = disc*V[i][k+1]
            if coefs[k] is not None:
                cont = sum(coefs[k][a]*paths[i][k]**a for a in range(deg+1))
            intr = max(X - paths[i][k], 0.0)
            V[i][k] = intr if intr > 0 and intr > cont else disc*V[i][k+1]
    return coefs

def lsm_low(paths, coefs, X, r, T, m):
    dt = T/m; tot = 0.0
    for p in paths:
        for k in range(1, m+1):
            if k == m:
                tot += max(X-p[m], 0.0)*math.exp(-r*m*dt); break
            cont = sum(coefs[k][a]*p[k]**a for a in range(len(coefs[k]))) if coefs[k] else 0.0
            ex = max(X - p[k], 0.0)
            if ex > 0 and ex >= cont:
                tot += ex*math.exp(-r*k*dt); break
    return tot/len(paths)

S0, X, T, r, sig, m = 100.0, 100.0, 1.0, 0.05, 0.20, 25
true = crr_am_put(S0, X, T, r, sig, 4000)
coefs = lsm_fit(simulate(S0, r, sig, T, m, 30000, 1), X, r, T, m, 2)
lowv  = lsm_low(simulate(S0, r, sig, T, m, 30000, 2), coefs, X, r, T, m)

def Vhat(k, x):
    h = max(X - x, 0.0)
    if k == m or coefs[k] is None: return h
    return max(h, sum(coefs[k][a]*x**a for a in range(3)))

# duality upper bound: E[ max_k ( payoff_k - M_k ) ], M a martingale from the value function
dt = T/m; drift = (r-0.5*sig**2)*dt; sg = sig*math.sqrt(dt); nj = 120; N = 5000
random.seed(9); tot = 0.0
for _ in range(N):
    x = S0; xs = [S0]
    for k in range(1, m+1):
        x *= math.exp(drift + sg*random.gauss(0,1)); xs.append(x)
    M = 0.0; best = 0.0
    for k in range(1, m+1):
        Ec = sum(Vhat(k, xs[k-1]*math.exp(drift+sg*random.gauss(0,1))) for _ in range(nj))/nj
        M += Vhat(k, xs[k]) - Ec
        best = max(best, max(X-xs[k], 0.0) - M)
    tot += best
ub = tot/N
print(f"true American put (CRR n=4000)             = {true:.4f}")
print(f"LSM lower bound (independent 2nd pass)     = {lowv:.4f}")
print(f"duality upper bound (value-fn martingale)  = {ub:.4f}")
print(f"interval [{lowv:.4f}, {ub:.4f}] contains true: {lowv <= true <= ub}")
```
```
true American put (CRR n=4000)             = 6.0902
LSM lower bound (independent 2nd pass)     = 6.0461
duality upper bound (value-fn martingale)  = 6.6440
interval [6.0461, 6.6440] contains true: True
```
...
- Forward: [[pillars/03-derivative-pricing/interest-rate-and-term-structure-models|Interest-Rate & Term-Structure Models]] (Bermudan swaptions are the archetypal high-dim American problem) · [[pillars/03-derivative-pricing/exotic-and-path-dependent-options/index|Exotic & Path-Dependent Options]]
