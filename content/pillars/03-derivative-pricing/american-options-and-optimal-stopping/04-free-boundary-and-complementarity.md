...
K, r, sig = 100.0, 0.10, 0.25
g = 2*r/sig**2; Lstar = g*K/(1.0+g)

# --- 1. Smooth pasting: the boundary L* maximises the value over constant boundaries ---
def vL(x, L): return (K-L)*(x/L)**(-g) if x > L else K-x
x = 90.0; best = max(((vL(x, 0.01*k), 0.01*k) for k in range(1, int(x*100))), key=lambda t: t[0])
print(f"argmax over L of V_L(90) = {best[1]:.4f}   closed-form L* = 2rK/(2r+sig^2) = {Lstar:.4f}")

# --- 2. Long-dated CRR converges to the perpetual value ---
def perp(x): return K-x if x <= Lstar else (K-Lstar)*(x/Lstar)**(-g)
print(f"perpetual closed form V(90) = {perp(90.0):.4f}")
for T in (5.0, 10.0, 20.0):
    print(f"  finite-T American put T={T:4.0f} (CRR n=2000) = {crr_am_put(90.0,K,T,r,sig,2000):.4f}")

# --- 3. Linear-complementarity residual  min(V-payoff, Lv) = 0  (finite differences) ---
def v(x): return K-x if x <= Lstar else (K-Lstar)*(x/Lstar)**(-g)
h = 1e-4
def Lop(x):
    vx = (v(x+h)-v(x-h))/(2*h); vxx = (v(x+h)-2*v(x)+v(x-h))/h**2
    return r*v(x) - r*x*vx - 0.5*sig**2*x*x*vxx
for x in (60.0, 75.0, 80.0, 100.0, 130.0):
    payoff = max(K-x, 0.0)
    print(f"  x={x:5.1f}: V-payoff={v(x)-payoff:+.4f}  Lv={Lop(x):+.4f}  min(V-payoff, Lv)={min(v(x)-payoff, Lop(x)):+.4f}")
```
```
argmax over L of V_L(90) = 76.1900   closed-form L* = 2rK/(2r+sig^2) = 76.1905
perpetual closed form V(90) = 13.9719
  finite-T American put T=   5 (CRR n=2000) = 13.4540
  finite-T American put T=  10 (CRR n=2000) = 13.8310
  finite-T American put T=  20 (CRR n=2000) = 13.9438
  x= 60.0: V-payoff=+0.0000  Lv=+10.0000  min(V-payoff, Lv)=+0.0000
  x= 75.0: V-payoff=+0.0000  Lv=+10.0000  min(V-payoff, Lv)=+0.0000
  x= 80.0: V-payoff=+0.3678  Lv=-0.0000  min(V-payoff, Lv)=-0.0000
  x=100.0: V-payoff=+9.9732  Lv=+0.0000  min(V-payoff, Lv)=+0.0000
  x=130.0: V-payoff=+4.3074  Lv=-0.0000  min(V-payoff, Lv)=-0.0000
```
