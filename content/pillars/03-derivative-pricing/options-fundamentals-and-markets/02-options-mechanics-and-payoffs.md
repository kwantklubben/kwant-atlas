...
def bsm(S,X,T,r,sig,kind):
    d1=(math.log(S/X)+(r+0.5*sig**2)*T)/(sig*math.sqrt(T)); d2=d1-sig*math.sqrt(T)
    return (S*N(d1)-X*math.exp(-r*T)*N(d2)) if kind=="c" else (X*math.exp(-r*T)*N(-d2)-S*N(-d1))
S,X,T,r,sig = 100.0,100.0,1.0,0.05,0.20
c,p = bsm(S,X,T,r,sig,"c"), bsm(S,X,T,r,sig,"p")
print(f"ATM S=100 X=100 T=1 r=5% sig=20%:  call={c:.4f}  put={p:.4f}   [BSM 10.4506 / 5.5735]")
print(f"  call = intrinsic {max(S-X,0):.2f} + time value {c-max(S-X,0):.4f}")
print(f"  put  = intrinsic {max(X-S,0):.2f} + time value {p-max(X-S,0):.4f}")
print("  moneyness (S=100):   X     call      put")
for Xk in (80.0,100.0,120.0):
    tagn = "ITM call/OTM put" if Xk<100 else ("ATM" if Xk==100 else "OTM call/ITM put")
    print(f"    {tagn:16s} {Xk:5.0f}  {bsm(S,Xk,T,r,sig,'c'):8.4f}  {bsm(S,Xk,T,r,sig,'p'):8.4f}")
K=100.0; prem=10.45
print(f"\n  long call (X={K:.0f}) net P&L diagram, premium {prem}:")
for ST in range(70,131,10):
    net = max(ST-K,0.0)-prem
    print(f"   S_T={ST:3d}: {net:+7.2f}  {'+' if net>=0 else ''}{'#'*int(round(abs(net)))}")
```
```
ATM S=100 X=100 T=1 r=5% sig=20%:  call=10.4506  put=5.5735   [BSM 10.4506 / 5.5735]
  call = intrinsic 0.00 + time value 10.4506
  put  = intrinsic 0.00 + time value 5.5735
  moneyness (S=100):   X     call      put
    ITM call/OTM put    80   24.5888    0.6872
    ATM                100   10.4506    5.5735
    OTM call/ITM put   120    3.2475   17.3950

  long call (X=100) net P&L diagram, premium 10.45:
   S_T= 70:  -10.45  ##########
   S_T= 80:  -10.45  ##########
   S_T= 90:  -10.45  ##########
   S_T=100:  -10.45  ##########
