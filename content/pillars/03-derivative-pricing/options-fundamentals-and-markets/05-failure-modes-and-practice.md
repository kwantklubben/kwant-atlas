...
import math, random
random.seed(2026)
n=20000; sigS,sigF,rho=0.0263,0.0313,0.928
dS=[]; dF=[]
for _ in range(n):
    z1=random.gauss(0,1); z2=rho*z1+math.sqrt(1-rho**2)*random.gauss(0,1)
    dS.append(sigS*z1); dF.append(sigF*z2)
hstar=rho*sigS/sigF
mf=sum(dF)/n; ms=sum(dS)/n
cov=sum((dS[i]-ms)*(dF[i]-mf) for i in range(n))/n
var=sum((dF[i]-mf)**2 for i in range(n))/n
print(f"(a) minimum-variance hedge ratio h* = rho*sigS/sigF (Hull eq 3.1)")
print(f"    theoretical = {hstar:.4f}   [Hull Ch3 airline example: 0.78]   empirical = {cov/var:.4f}")
print(f"    hedge effectiveness = R^2 = rho^2 = {rho**2:.4f}  -> {rho**2*100:.2f}% of variance removed")
resid=(1-rho**2)*sigS**2
print(f"    residual variance = (1-rho^2) sigS^2 = {resid:.8f} (std {math.sqrt(resid):.6f})")
print(f"(b) contract count N* = h* Q_A/Q_F (Hull eq 3.2): {hstar:.4f} x 2,000,000/42,000 = {hstar*2_000_000/42_000:.2f} -> {round(hstar*2_000_000/42_000)} contracts")
F1,S2,F2 = 2.20, 2.15, 2.10
print(f"(c) effective hedged price = F1 + basis b2 = {F1} + {S2-F2:+.2f} = {F1 + (S2-F2):.4f}   [Hull Sec 3.3]")
tailed = round(hstar*2_000_000/42_000)/1.05
print(f"(d) tailing the hedge (1-yr, r=5%): N*/1.05 = {tailed:.2f}")
```
```
(a) minimum-variance hedge ratio h* = rho*sigS/sigF (Hull eq 3.1)
    theoretical = 0.7798   [Hull Ch3 airline example: 0.78]   empirical = 0.7808
    hedge effectiveness = R^2 = rho^2 = 0.8612  -> 86.12% of variance removed
    residual variance = (1-rho^2) sigS^2 = 0.00009602 (std 0.009799)
(b) contract count N* = h* Q_A/Q_F (Hull eq 3.2): 0.7798 x 2,000,000/42,000 = 37.13 -> 37 contracts
(c) effective hedged price = F1 + basis b2 = 2.2 + +0.05 = 2.2500   [Hull Sec 3.3]
(d) tailing the hedge (1-yr, r=5%): N*/1.05 = 35.24
```
