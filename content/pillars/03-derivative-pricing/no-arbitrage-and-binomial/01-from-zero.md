...
Su, Sd = S0*u, S0*d
Vu, Vd = max(K-Su, 0.0), max(K-Sd, 0.0)          # put payoffs: 0 and 3

delta = (Vu - Vd) / (Su - Sd)                     # (1.6)
pt = (1 + r - d) / (u - d); qt = (u - 1 - r) / (u - d)   # (1.8)
V0 = (pt*Vu + qt*Vd) / (1 + r)                    # (1.9)
B  = V0 - delta*S0                                # bond position retained

print(f"S1(H)={Su:.1f} S1(T)={Sd:.1f}   V1(H)={Vu:.1f} V1(T)={Vd:.1f}")
print(f"delta={delta:.4f}  V0={V0:.4f}  bond={B:.4f}")
print(f"replicate H: delta*Su+(1+r)*B = {delta*Su+(1+r)*B:.4f}  (payoff {Vu:.4f})")
print(f"replicate T: delta*Sd+(1+r)*B = {delta*Sd+(1+r)*B:.4f}  (payoff {Vd:.4f})")
print(f"p_tilde={pt:.4f}  q_tilde={qt:.4f}  sum={pt+qt:.4f}")
print(f"bracket: d={d:.2f} < 1+r={1+r:.2f} < u={u:.2f} ? {d < 1+r < u}")
```
```
S1(H)=8.0 S1(T)=2.0   V1(H)=0.0 V1(T)=3.0
delta=-0.5000  V0=1.2000  bond=3.2000
replicate H: delta*Su+(1+r)*B = 0.0000  (payoff 0.0000)
replicate T: delta*Sd+(1+r)*B = 3.0000  (payoff 3.0000)
p_tilde=0.5000  q_tilde=0.5000  sum=1.0000
bracket: d=0.50 < 1+r=1.25 < u=2.00 ? True
```
...
- Base: [[foundations/probability-and-measure-theory|Probability & Measure Theory]] · [[pillars/03-derivative-pricing/no-arbitrage-and-binomial-trees|No-Arbitrage Foundations & Binomial Trees]]
