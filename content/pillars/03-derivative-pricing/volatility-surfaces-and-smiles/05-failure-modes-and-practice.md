...
# (i) butterfly arbitrage: an over-steep, over-curved SVI slice
Ab,Bb,rhob,sb=0.04,0.9,-0.9,0.01
def svi_bad(k): return Ab+Bb*(rhob*k+math.sqrt(k*k+sb*sb))
def density_bad(k,T=1.0,F=1.0):              # Breeden-Litzenberger g(K)=d2C/dK2
    K=F*math.exp(k)
    def C(K): return bsm_call(F,K,T,0.0,math.sqrt(max(svi_bad(math.log(K/F)),1e-8)))[0]
    h=1e-3;dK=K*h
    return (C(K*(1+h))-2*C(K)+C(K*(1-h)))/(dK*dK)
ks=[i*0.02-0.8 for i in range(81)]; dv=[density_bad(k) for k in ks]
print(f"bad SVI slice: min density = {min(dv):.6f} at k={ks[dv.index(min(dv))]:+.2f} -> butterfly arbitrage")

# (iii) LV forward-skew flattening (Bergomi 2.91/2.92), power-law skew S_t = alpha (t/tau)^-gamma
def S_term(t,alpha=0.4,tau=0.15,gamma=0.5): return alpha*(max(t,tau)/tau)**(-gamma)
def S_fwd(tau0,theta,gamma=0.5,n=20000):
    a,b=tau0,tau0+theta;tot=0.0;dt=(b-a)/n
    for i in range(n):
        t=a+(i+0.5)*dt;tot+=S_term(t,gamma=gamma)*dt
    return S_term(b,gamma=gamma)-(tau0/theta)*(tot/theta-S_term(b,gamma=gamma))
print(f"LV forward skew: S_1(today)={S_term(1.0):.4f}  fwd 1y-in-1y={S_fwd(1.0,1.0):.4f}  ratio={S_fwd(1.0,1.0)/S_term(1.0):.3f}")

# (ii) calendar arbitrage check helper on total variance w(k,T)
def calendar_ok(w,k,T1,T2): return w(k,T2) >= w(k,T1)
w_good=lambda k,T: 0.04*T
w_bad =lambda k,T: 0.04*T if T<0.5 else 0.02-0.005*(T-0.5)
print(f"good surface calendar_ok(0.5,1.0) = {calendar_ok(w_good,0.0,0.5,1.0)}   (w={w_good(0,1.0):.4f})")
print(f"bad  surface calendar_ok(0.5,1.0) = {calendar_ok(w_bad ,0.0,0.5,1.0)}   (w={w_bad(0,1.0):.4f} < {w_bad(0,0.5):.4f})")
```
```
bad SVI slice: min density = -6.922689 at k=-0.04 -> butterfly arbitrage
LV forward skew: S_1(today)=0.1549  fwd 1y-in-1y=0.0907  ratio=0.586
good surface calendar_ok(0.5,1.0) = True   (w=0.0400)
bad  surface calendar_ok(0.5,1.0) = False   (w=0.0175 < 0.0200)
```
