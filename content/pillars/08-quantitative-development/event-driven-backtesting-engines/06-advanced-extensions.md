...
# ===== 1. Latency: tail, not mean, is the cost =====
random.seed(11)
N = 200_000
tight = [max(0.0, random.gauss(50, 5)) for _ in range(N)]        # tight: 50us, sd 5us
sigma = 0.8; mu = math.log(50) - 0.5*sigma**2                     # lognormal, same ~50us mean
fat = [random.lognormvariate(mu, sigma) for _ in range(N)]

def pct(xs, p):
    ys = sorted(xs); return ys[min(int(p/100*len(ys)), len(ys)-1)]

print("Wire-latency distributions (microseconds), same ~50us mean:")
print(f"{'dist':>10} {'mean':>7} {'p50':>8} {'p99':>8} {'p99.9':>9} {'p99.9/p50':>10}")
for name, xs in (("tight", tight), ("fat-tail", fat)):
    m = sum(xs)/len(xs)
    print(f"{name:>10} {m:>7.1f} {pct(xs,50):>8.1f} {pct(xs,99):>8.1f} {pct(xs,99.9):>9.1f}"
          f" {pct(xs,99.9)/pct(xs,50):>9.1f}x")

# ===== 2. Queue position -> passive fill probability =====
def fill_prob(q_shares, avg_trade, lam, tau):
    """P(queue ahead of q is exhausted within tau) under Poisson arrivals."""
    need = math.ceil(q_shares/avg_trade); mean = lam*tau
    p_less = sum(math.exp(-mean)*mean**k/math.factorial(k) for k in range(need))
    return 1.0 - p_less

print("\nPassive-fill probability vs queue depth (lambda=5 trades/s, avg trade=100 sh, tau=2s):")
print(f"{'shares ahead':>13} {'P(fill) in 2s':>14}")
for q in (0, 250, 500, 1000, 2000, 4000):
    print(f"{q:>13,} {fill_prob(q, 100, 5.0, 2.0):>13.3f}")
```
```
Wire-latency distributions (microseconds), same ~50us mean:
      dist    mean      p50      p99     p99.9  p99.9/p50
     tight    50.0     50.0     61.5      65.1       1.3x
  fat-tail    49.9     36.3    229.9     436.8      12.0x

Passive-fill probability vs queue depth (lambda=5 trades/s, avg trade=100 sh, tau=2s):
 shares ahead  P(fill) in 2s
            0         1.000
          250         0.997
          500         0.971
        1,000         0.542
