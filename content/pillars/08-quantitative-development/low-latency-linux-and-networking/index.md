...
def build(stage_sigmas, n=200000):
    med_base = {"wire":3.0,"nic":1.0,"stack":8.0,"parse":2.0}   # us medians
    tot = []
    for _ in range(n):
        t = 0.0
        for k, med in med_base.items():
            s = stage_sigmas[k]
            t += math.exp(random.gauss(math.log(med), s))       # log-normal stage
        tot.append(t)
    return tot
cfg_kernel = {"wire":0.3,"nic":0.3,"stack":1.1,"parse":0.4}     # jittery kernel stack
cfg_bypass = {"wire":0.3,"nic":0.3,"stack":0.25,"parse":0.4}    # bypass: stack tamed
for name, cfg in (("kernel (jittery stack)", cfg_kernel),
                  ("bypass (flat stack)",  cfg_bypass)):
    xs = build(cfg)
    print(f"{name:24s} mean={sum(xs)/len(xs):5.2f} p50={pct(xs,50):5.2f} "
          f"p99={pct(xs,99):6.2f} p99.9={pct(xs,99.9):7.2f} us  "
          f"p99/p50={pct(xs,99)/pct(xs,50):.2f}x")
```
```
kernel (jittery stack)   mean=21.01 p50=14.49 p99=110.64 p99.9= 247.02 us  p99/p50=7.63x
bypass (flat stack)      mean=14.61 p50=14.38 p99= 21.53 p99.9=  24.56 us  p99/p50=1.50x
```
