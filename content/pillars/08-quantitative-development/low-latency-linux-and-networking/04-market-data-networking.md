...
iat = []; seq = []; gaps = []
for i in range(n):
    gap = random.expovariate(1/1.2) if random.random() < 0.7 else random.expovariate(1/9.0)
    iat.append(gap)                     # bursty: 1.2us inside a burst, 9us sparse
    if gap > 25: gaps.append(i)         # jitter event: long inter-arrival
print(f"simulated {n} messages, nominal ~100k msg/s")
print(f"mean IAT={sum(iat)/len(iat):5.2f} us, p99 IAT={pct(iat,99):6.2f} us, "
      f"max IAT={max(iat):6.1f} us")
print(f"large inter-arrival gaps (>25us, jitter events): {len(gaps)} "
      f"({len(gaps)/n*100:.2f}% of messages)")

seq = list(range(n))
drop = set(random.sample(seq, 3))       # 3 lost packets
recv = [s for s in seq if s not in drop]
detections = 0
for a, b in zip(recv, recv[1:]):
    if b - a > 1:
        detections += 1
print(f"3 dropped msgs: sequence-gap detection fired {detections} time(s), "
      f"all {len(drop)} lost messages detected")
```
```
