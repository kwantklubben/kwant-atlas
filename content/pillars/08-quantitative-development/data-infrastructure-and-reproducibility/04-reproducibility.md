...
def digest(x):
    h = hashlib.sha256(repr(x).encode()); return h.hexdigest()

r1 = run_pipeline(99)
r2 = run_pipeline(99)               # same code, same seed, same result
print(f"run #1 result = {r1:.10f}  hash {digest(r1)[:16]}")
print(f"run #2 result = {r2:.10f}  hash {digest(r2)[:16]}")
print("bit-identical across reruns:", r1 == r2)

r3 = run_pipeline(100)              # seed changed => result must differ
print(f"seed-100  result = {r3:.10f}  differs: {r3 != r1}")
```
```
run #1 result = 99962.0806309454  hash de9d1357f0c9ac97
run #2 result = 99962.0806309454  hash de9d1357f0c9ac97
bit-identical across reruns: True
seed-100  result = 100038.3444348047  differs: True
```
...
```python
import threading
shards = [1e16, 1.0, -1e16, 1.0]   # four shard partial sums from a parallel reduce

tot_a = shards[0]+shards[1]+shards[2]+shards[3]   # ((1e16+1) - 1e16) + 1
tot_b = shards[3]+shards[2]+shards[1]+shards[0]   # reversed reduction order

print(f"order A ((s0+s1)+s2)+s3 = {tot_a:.1f}")
print(f"order B ((s3+s2)+s1)+s0 = {tot_b:.1f}")
print("floating-point reduction order changes the result (A != B):", tot_a != tot_b)
```
```
order A ((s0+s1)+s2)+s3 = 1.0
order B ((s3+s2)+s1)+s0 = 0.0
floating-point reduction order changes the result (A != B): True
```
