...
```python
# Latency budget for one tick-to-trade round trip (microseconds)
stages = {
    "NIC / kernel-bypass receive": 0.15,
    "wire decode (binary feed)":   0.30,
    "order-book update":           0.20,
    "strategy signal":             0.80,
    "pre-trade risk":              0.25,
    "order serialize + CRC":       0.15,
    "NIC transmit":                0.15,
}
total = sum(stages.values())
print("Tick-to-trade latency budget (microseconds):")
for k, v in stages.items():
    bar = "#" * int(round(v / 0.05))
    print(f"  {k:28s} {v:5.2f} us  {bar} {100*v/total:5.1f}%")
print(f"  {'TOTAL':28s} {total:5.2f} us")

# Alpha decay: half of a short-horizon edge is gone every `half_life` us.
half_life = 10.0
print(f"\nEdge retained vs reaction half-life {half_life:.0f} us:")
for lat in (1.0, total, 10.0, 100.0):
    frac = 0.5 ** (lat / half_life)
    print(f"  reaction latency {lat:7.2f} us -> alpha surviving {frac*100:6.2f}%")

# Throughput: requests serviced per second if fully pipelined (Little's law style)
per_sec = 1_000_000 / total
print(f"\nFully-pipelined ceiling at this latency: {per_sec:,.0f} msg/s "
      f"(assuming {total:.2f} us of work per message, no parallelism)")
```
```
Tick-to-trade latency budget (microseconds):
  NIC / kernel-bypass receive   0.15 us  ###   7.5%
  wire decode (binary feed)     0.30 us  ######  15.0%
  order-book update             0.20 us  ####  10.0%
  strategy signal               0.80 us  ################  40.0%
  pre-trade risk                0.25 us  #####  12.5%
  order serialize + CRC         0.15 us  ###   7.5%
  NIC transmit                  0.15 us  ###   7.5%
  TOTAL                         2.00 us

Edge retained vs reaction half-life 10 us:
  reaction latency    1.00 us -> alpha surviving  93.30%
  reaction latency    2.00 us -> alpha surviving  87.06%
  reaction latency   10.00 us -> alpha surviving  50.00%
  reaction latency  100.00 us -> alpha surviving   0.10%

Fully-pipelined ceiling at this latency: 500,000 msg/s (assuming 2.00 us of work per message, no parallelism)
```
