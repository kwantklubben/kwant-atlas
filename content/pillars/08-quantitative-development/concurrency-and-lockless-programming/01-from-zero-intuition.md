...
def amdahl(P, f_serial):
    """Maximum speedup with P processors when fraction f_serial of the work
       is unavoidably serial (Amdahl's law)."""
    return 1.0 / (f_serial + (1.0 - f_serial) / P)

print("Amdahl's law speedup ceiling  S(P) = 1 / (f + (1-f)/P)")
print(f"{'P (cores)':>10s}  {'f=0.01':>10s} {'f=0.05':>10s} {'f=0.20':>10s}")
for P in (1, 2, 4, 8, 16, 64, 1024):
    row = f"{P:10d}"
    for f in (0.01, 0.05, 0.20):
        row += f"  {amdahl(P, f):10.2f}"
    print(row)

print("\nAsymptotic ceiling as P->inf is exactly 1/f_serial:")
for f in (0.01, 0.05, 0.20):
    print(f"  f_serial={f*100:4.1f}%  ->  speedup ceiling = {1.0/f:.1f}x")
```
```
Amdahl's law speedup ceiling  S(P) = 1 / (f + (1-f)/P)
 P (cores)      f=0.01     f=0.05     f=0.20
         1        1.00        1.00        1.00
         2        1.98        1.90        1.67
         4        3.88        3.48        2.50
         8        7.48        5.93        3.33
