...
```python
def false_sharing_model(K, a=5.0, T=40.0):
    """Cost model for two threads each doing K increments of a counter.
       a = ns per uncontended local increment; T = ns of cache-coherence
       transfer when the other thread must invalidate/refetch the shared line."""
    false_ms = 2 * K * (a + T)   # shared line: increments serialise on the bounce
    padded_ms = K * a            # separate lines: threads run in parallel
    return false_ms, padded_ms, false_ms / padded_ms

print("False-sharing cost model  (a=5 ns local increment, T=40 ns line transfer)")
print(f"{'K (incs/thread)':>16s}  {'false-shared':>14s}  {'padded':>10s}  {'slowdown':>9s}")
for K in (1_000, 100_000, 1_000_000):
    f, p, ratio = false_sharing_model(K)
    print(f"{K:16,d}  {f/1e3:13,.0f} us  {p/1e3:9,.0f} us  {ratio:8.1f}x")

K = 1_000_000
f, p, ratio = false_sharing_model(K)
print(f"\nHeadline (K={K:,}): false-shared = {f/1e3:,.0f} us vs padded = {p/1e3:,.0f} us "
      f"-> {ratio:.1f}x slowdown from one shared cache line.")
```
```
False-sharing cost model  (a=5 ns local increment, T=40 ns line transfer)
 K (incs/thread)    false-shared      padded   slowdown
           1,000             90 us          5 us      18.0x
