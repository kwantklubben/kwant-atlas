...
```python
# --- the gap between a backtest and a live book (stdlib only) ---
gross_ann, vol_ann = 0.12, 0.06          # 12%/yr gross return, 6%/yr vol -> Sharpe 2.0
cost_bps = 6.0                           # one-way: half-spread + impact, per unit traded

print("Turnover tax (gross 12%/yr, vol 6%, cost 6 bps one-way)")
print(f"{'turnover/yr':>12s} {'drag':>8s} {'net ret':>9s} {'net Sharpe':>11s}")
for turnover in (50, 100, 200, 300, 600, 1200):
    drag = turnover * cost_bps / 1e4
    net = gross_ann - drag
    print(f"{turnover:12d} {drag*100:7.2f}% {net*100:8.2f}% {net/vol_ann:11.2f}")

break_even = gross_ann * 1e4 / cost_bps
print(f"\nBreak-even turnover (drag = edge): {break_even:,.0f} per year")
net_100 = gross_ann - 100 * cost_bps / 1e4
print(f"At 100x/yr net return = {net_100*100:.2f}% -> net Sharpe {net_100/vol_ann:.2f} "
      f"({gross_ann/vol_ann:.2f} backtested)")

# --- one production incident erases years of edge ---
incident_loss = 0.30
print(f"\nA single -30% production incident erases "
      f"{incident_loss/net_100:.2f} years of net edge")
```
```
Turnover tax (gross 12%/yr, vol 6%, cost 6 bps one-way)
 turnover/yr     drag   net ret  net Sharpe
          50    3.00%     9.00%        1.50
         100    6.00%     6.00%        1.00
         200   12.00%     0.00%        0.00
         300   18.00%    -6.00%       -1.00
         600   36.00%   -24.00%       -4.00
        1200   72.00%   -60.00%      -10.00

Break-even turnover (drag = edge): 200 per year
At 100x/yr net return = 6.00% -> net Sharpe 1.00 (2.00 backtested)

A single -30% production incident erases 5.00 years of net edge
```
