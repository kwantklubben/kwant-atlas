---
title: "06 - Advanced Extensions: High Availability, Failover & Cluster-Scale Safety"
tags:
  - pillar-quant-dev
...
print(f"  annual downtime = {down_h:.3f} h = {down_h*60:.1f} min")

for n in (2, 3):
    An = 1.0 - (1.0 - A) ** n
    print(f"  {n} independent replicas: A = {An:.10f}  downtime = "
          f"{(1-An)*8760*3600*1000:.2f} ms/yr")

# common-cause fraction: outages no amount of redundancy removes
for f in (0.1, 0.5):
    d = (1.0 - A) * (f + (1.0 - f) * (1.0 - A))     # n = 2
    print(f"  n=2 with common-cause fraction {f:.0%}: downtime = {d*8760:.3f} h/yr")

# failover timeline: heartbeat interval h, timeout = 4 missed beats
beat, k = 0.5, 4
timeout = beat * k
detect = timeout / 2.0 + beat / 2.0
promote = 1.5                                   # seconds to fence + replay + start
rto = detect + promote
print(f"\nFailover control plane: heartbeat {beat}s, timeout {timeout}s")
print(f"  expected detection {detect:.2f}s + promotion {promote:.2f}s "
      f"=> RTO {rto:.2f}s")
print(f"  a tighter timeout cuts RTO but raises false failovers: "
      f"{beat:.2f}s miss rate risk vs {timeout:.1f}s blackout tolerance")
```
```
Single runtime: A = 0.99965290  (99.96529%)
  annual downtime = 3.041 h = 182.4 min
  2 independent replicas: A = 0.9999998795  downtime = 3799.44 ms/yr
  3 independent replicas: A = 1.0000000000  downtime = 1.32 ms/yr
  n=2 with common-cause fraction 10%: downtime = 0.305 h/yr
  n=2 with common-cause fraction 50%: downtime = 1.521 h/yr

Failover control plane: heartbeat 0.5s, timeout 2.0s
  expected detection 1.25s + promotion 1.50s => RTO 2.75s
  a tighter timeout cuts RTO but raises false failovers: 0.50s miss rate risk vs 2.0s blackout tolerance
```
