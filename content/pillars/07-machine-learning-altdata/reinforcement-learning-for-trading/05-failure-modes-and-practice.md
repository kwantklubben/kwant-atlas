...
**Basic Prerequisites:** [[pillars/07-machine-learning-altdata/reinforcement-learning-for-trading/04-policy-gradient-and-actor-critic|04 · Policy-Gradient & Actor-Critic]] and [[pillars/07-machine-learning-altdata/financial-ml-pitfalls-and-low-snr|Financial ML Pitfalls & Low SNR]].
...
```python
P, gamma, N = 10.0, 0.9, 3
def gross(x, a, k=2.0, h=1.0): return 0.0 if x == 0 else a*P              # ignores impact/holding
def net(x, a, k=2.0, h=1.0):   return 0.0 if x == 0 else a*P - k*a*a - h*x

def vi(rew, k=2.0, h=1.0):
    V = [0.0]*(N+1)
    for _ in range(500):
        V = [max(rew(x, a, k, h) + gamma*V[x-a] for a in range(x+1)) for x in range(N+1)]
    pol = [max(range(x+1), key=lambda a: rew(x, a, k, h) + gamma*V[x-a]) for x in range(N+1)]
    return V, pol

def eval_policy(pol, k=2.0, h=1.0):        # value of a policy under the TRUE (net) objective
    x, G, disc = N, 0.0, 1.0
    while x > 0:
        a = pol[x]; G += disc*net(x, a, k, h); disc *= gamma; x -= a
    return G

Vg, pol_gross = vi(gross)
Vn, pol_net   = vi(net)
print("EXP1 reward hacking")
print(f"  policy trained on GROSS reward : {pol_gross}  -> realized NET value = {eval_policy(pol_gross):.4f}")
print(f"  policy trained on NET   reward : {pol_net}  -> realized NET value = {eval_policy(pol_net):.4f}")
print(f"  cost of the wrong reward function = {eval_policy(pol_net)-eval_policy(pol_gross):.4f}")

# ---- Non-stationarity: expect the price to RISE (train) vs FALL (deploy) ----
T, X0, kk, S0 = 3, 3, 1.0, 10.0
def vi_drift(mu):
    V = {(T, x): (-1e9*x if x > 0 else 0.0) for x in range(X0+1)}; pol = {}
    for t in range(T-1, -1, -1):
        for x in range(X0+1):
            best, besta = -1e18, 0
            for a in range(x+1):
                val = a*(S0 + mu*t) - kk*a*a + gamma*V[(t+1, x-a)]
                if val > best: best, besta = val, a
            V[(t, x)] = best; pol[(t, x)] = besta
    return pol

def eval_drift(pol, mu):
    x, G, disc = X0, 0.0, 1.0
    for t in range(T):
        a = pol[(t, x)]; G += disc*(a*(S0 + mu*t) - kk*a*a); disc *= gamma; x -= a
    return G + (-1e9*x if x > 0 else 0.0)

pA, pB = vi_drift(+0.5), vi_drift(-0.5)
print("\nEXP2 non-stationarity (drift +0.5 -> -0.5)")
print(f"  policy trained on mu=+0.5 : value  = {eval_drift(pA, -0.5):.4f}")
print(f"  oracle policy for mu=-0.5 : value  = {eval_drift(pB, -0.5):.4f}")
print(f"  loss from a stale policy  = {eval_drift(pB,-0.5)-eval_drift(pA,-0.5):.4f}")
```
```
EXP1 reward hacking
  policy trained on GROSS reward : [0, 1, 2, 3]  -> realized NET value = 9.0000
  policy trained on NET   reward : [0, 1, 1, 1]  -> realized NET value = 16.0700
  cost of the wrong reward function = 7.0700

EXP2 non-stationarity (drift +0.5 -> -0.5)
  policy trained on mu=+0.5 : value  = 23.1300
  oracle policy for mu=-0.5 : value  = 23.6500
  loss from a stale policy  = 0.5200
```
...
- Hygiene cousins: [[pillars/07-machine-learning-altdata/financial-ml-pitfalls-and-low-snr/05-failure-modes-and-practice|Financial ML Pitfalls · 05 · Failure Modes]] · [[pillars/01-quantitative-research/backtesting-hygiene-and-deflated-sharpe|Backtesting Hygiene & Deflated Sharpe]]
- Non-stationarity: [[pillars/07-machine-learning-altdata/financial-ml-pitfalls-and-low-snr/04-non-stationarity-and-samples|Non-Stationarity & Samples]] · [[pillars/07-machine-learning-altdata/regime-classification-hmm-and-gmm|Regime Classification: HMM & GMM]]
- Execution baseline to beat: [[pillars/02-algorithmic-hft/optimal-execution-and-almgren-chriss|Optimal Execution & Almgren–Chriss]] · [[pillars/05-portfolio-optimization/constraints-and-transaction-costs|Constraints & Transaction Costs]]
