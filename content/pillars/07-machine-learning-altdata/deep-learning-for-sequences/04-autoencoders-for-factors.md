...
# ---- data: rank-k factor structure in d dims PLUS idiosyncratic noise ----
d, n, k = 10, 800, 3
B = np.random.randn(d, k)                      # true loading matrix
X = (B @ np.random.randn(k, n)).T + 0.5*np.random.randn(n, d)   # rank k + noise
X = X - X.mean(0)                              # centre

# ---- (A) PCA rank-k reconstruction (Eckart-Young: the linear optimum) ----
U, S, Vt = np.linalg.svd(X, full_matrices=False)
pca_recon = U[:, :k] @ np.diag(S[:k]) @ Vt[:k]
pca_mse = ((X - pca_recon)**2).mean()

# ---- (B) linear autoencoder d->k->d trained by gradient descent ----
def train_ae(X, k, steps=6000, lr=0.05, seed=0):
    rng = np.random.default_rng(seed)
    d = X.shape[1]
    E = rng.normal(0, 0.1, (k, d))             # encoder  z = E x
    D = rng.normal(0, 0.1, (d, k))             # decoder  xhat = D z
    for _ in range(steps):
        Z = X @ E.T                            # n x k code
        R = Z @ D.T                            # n x d reconstruction
        err = R - X
        E -= lr * ((err @ D).T @ X / len(X))   # dL/dE
        D -= lr * (err.T @ Z / len(X))         # dL/dD
    return E, D

E, D = train_ae(X, k)
ae_mse = ((X - X @ E.T @ D.T)**2).mean()

print("dimensions: n=%d samples, d=%d features, bottleneck k=%d" % (n, d, k))
print("PCA rank-%d reconstruction MSE   = %.4f  (linear optimum)" % (k, pca_mse))
print("trained linear autoencoder MSE  = %.4f" % ae_mse)
print("relative gap (AE vs PCA optimum) = %+.3f%%" % (100*(ae_mse-pca_mse)/pca_mse))
print("total variance  = %.4f ;  Rank-%d PCA captures %.1f%%" %
      (X.var(0).sum(), k, 100*(1 - pca_mse/X.var(0).sum())))

# subspace alignment: principal angles between AE encoder rows and PCA subspace
Qp, _ = np.linalg.qr(Vt[:k].T)                 # PCA loading subspace
Qa, _ = np.linalg.qr(E.T)                      # AE encoder subspace
sv = np.linalg.svd(Qp.T @ Qa, compute_uv=False)
print("cos(principal angles) AE-vs-PCA  =", np.round(sv, 4))
```
```
dimensions: n=800 samples, d=10 features, bottleneck k=3
PCA rank-3 reconstruction MSE   = 0.1714  (linear optimum)
trained linear autoencoder MSE  = 0.1714
relative gap (AE vs PCA optimum) = -0.000%
total variance  = 41.5775 ;  Rank-3 PCA captures 99.6%
cos(principal angles) AE-vs-PCA  = [1. 1. 1.]
```
...
- Low-SNR reality check: [[pillars/07-machine-learning-altdata/financial-ml-pitfalls-and-low-snr|Financial ML Pitfalls & Low SNR]]
