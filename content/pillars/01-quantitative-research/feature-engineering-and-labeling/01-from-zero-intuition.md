...
# Fixed-time-horizon label = sign of return over H bars.
# Triple-barrier label = which barrier is touched FIRST.
def tb_label(path, pt, sl):
    p0 = path[0]; up, dn = p0*(1+pt), p0*(1-sl)
    for p in path[1:]:
        if p >= up: return 1
        if p <= dn: return -1
    return 1 if path[-1] > p0 else -1     # vertical barrier: sign of return

random.seed(7)
H, sigma, n = 20, 0.02, 30000
dis, example = 0, None
pt = sl = 0.5*sigma*math.sqrt(H)          # 0.5-sigma barriers (bind often)
for _ in range(n):
    p = 100.0; path = [p]
    for _ in range(H):
        p *= math.exp(-0.5*sigma**2 + sigma*random.gauss(0, 1)); path.append(p)
    fh = 1 if path[-1] > path[0] else -1
    tr = tb_label(path, pt, sl)
    if fh != tr:
        dis += 1
        if example is None: example = (fh, tr, path)

print(f"H={H} bars, sigma={sigma:.0%}, barriers at 0.5*sigma*sqrt(H) = {pt:.1%}; n={n}")
print(f"fixed-horizon sign disagrees with triple-barrier label in {dis/n:.1%} of paths")
fh, tr, p = example
print(f"example (fixed-horizon={fh:+d}, triple-barrier={tr:+d}):")
print("  path: " + " ".join(f"{x:.1f}" for x in p[::4]))
print(f"  upper={p[0]*(1+pt):.2f}  lower={p[0]*(1-pt):.2f}  max={max(p):.2f}  min={min(p):.2f}  last={p[-1]:.2f}")
```
```
H=20 bars, sigma=2%, barriers at 0.5*sigma*sqrt(H) = 4.5%; n=30000
fixed-horizon sign disagrees with triple-barrier label in 19.7% of paths
example (fixed-horizon=-1, triple-barrier=+1):
  path: 100.0 101.4 101.9 99.2 96.8 99.5
  upper=104.47  lower=95.53  max=106.21  min=96.82  last=99.51
```
