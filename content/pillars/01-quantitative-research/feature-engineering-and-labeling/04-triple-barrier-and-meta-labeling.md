...
# A CONTROLLED experiment: the feature z genuinely carries signal about whether
# the primary bet wins, so we can isolate the meta-labeling mechanism itself.
def logit_fit(X, y, lr=0.5, epochs=3000, l2=1e-3):
    k = len(X[0]); b = [0.0]*k
    for _ in range(epochs):
        g = [0.0]*k
        for xi, yi in zip(X, y):
            z = sum(b[j]*xi[j] for j in range(k)); p = 1/(1+math.exp(-z))
            for j in range(k): g[j] += (p-yi)*xi[j]
        for j in range(k): b[j] -= lr*(g[j]/len(X) + l2*b[j])
    return b

random.seed(23)
N = 4000
z   = [random.gauss(0, 1) for _ in range(N)]
# meta-label: 1 iff the primary bet wins; win prob rises with z (the secondary feature)
y   = [1 if random.random() < 1/(1+math.exp(-1.2*z[i])) else 0 for i in range(N)]
split = int(0.6*N)                                  # time-ordered split
X = [[1.0, z[i]] for i in range(N)]
b = logit_fit(X[:split], y[:split])
p_hat = [1/(1+math.exp(-(b[0]+b[1]*X[i][1]))) for i in range(N)]

def scores(idx, sel):
    tp = sum(1 for i in idx if y[i] == 1 and sel[i])
    fp = sum(1 for i in idx if y[i] == 0 and sel[i])
    fn = sum(1 for i in idx if y[i] == 1 and not sel[i])
    prec = tp/(tp+fp) if tp+fp else 0.0; rec = tp/(tp+fn) if tp+fn else 0.0
    f1 = 2*prec*rec/(prec+rec) if prec+rec else 0.0
    return tp+fp, prec, rec, f1

te = range(split, N)
nb0, p0, r0, f0 = scores(te, [True]*N)                       # primary: bet everything
for thr in (0.5, 0.6, 0.7):
    sel = [p_hat[i] >= thr if i >= split else False for i in range(N)]
    nb, p, r, f = scores(te, sel)
    print(f"secondary threshold P>={thr:.1f}: n_bet={nb:4d}  precision={p:.3f}  "
          f"recall={r:.3f}  F1={f:.3f}   (filtered {nb0-nb} bets)")
print(f"primary (bet always) : n_bet={nb0:4d}  precision={p0:.3f}  recall={r0:.3f}  F1={f0:.3f}")
print(f"learned LR coefficients [bias, z] = [{b[0]:+.3f}, {b[1]:+.3f}]  (true slope 1.2)")
```
```
secondary threshold P>=0.5: n_bet= 750  precision=0.721  recall=0.653  F1=0.686   (filtered 850 bets)
secondary threshold P>=0.6: n_bet= 535  precision=0.794  recall=0.513  F1=0.624   (filtered 1065 bets)
secondary threshold P>=0.7: n_bet= 322  precision=0.829  recall=0.322  F1=0.464   (filtered 1278 bets)
primary (bet always) : n_bet=1600  precision=0.517  recall=1.000  F1=0.682
learned LR coefficients [bias, z] = [-0.118, +1.174]  (true slope 1.2)
```
...
- Cross-pillar: [[pillars/07-machine-learning-altdata/tree-based-factor-ranking-and-purged-cv|Tree-Based Factor Ranking & Purged CV]]
