---
title: "05 — Failure Modes & Real-World Practice: The Hygiene Checklist"
tags:
...
If the strategy turns over fraction $\tau$ of its book per period and faces round-trip cost $c$ (proportional spread + fees), the net per-period return is $\tilde r_t = r_t - \tau c$, so the net Sharpe is
...
$$\underbrace{\text{DSR}\ge0.95}_{\text{selection bias + non-normality}}\ \wedge\ \underbrace{\widehat{SR}_{\text{net}}>0\ \text{after full costs}}_{\text{accounting}}\ \wedge\ \underbrace{T\ge\text{MinTRL}}_{\text{sample sufficiency}}\ \wedge\ \underbrace{\text{PBO}\le0.5}_{\text{selection robustness (page 06).}}$$
