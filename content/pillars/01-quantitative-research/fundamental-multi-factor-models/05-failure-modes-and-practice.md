---
title: "05 — Failure Modes & Real-World Practice: Crowding, Multicollinearity, the Factor Zoo"
tags:
...
def ols_stats(X, y):
    ...
    resid = [y[t]-sum(coef[j]*X[t][j] for j in range(p)) for t in range(n)]
    s2 = sum(e*e for e in resid)/(n-p)
    Ainv = [[0.0]*p for _ in range(p)]
    for i in range(p):
        e = [0.0]*p; e[i] = 1.0
        MM = [A[row][:] + [e[row]] for row in range(p)]
        for col in range(p):
            piv = max(range(col, p), key=lambda r: abs(MM[r][col])); MM[col], MM[piv] = MM[piv], MM[col]
            for r in range(p):
                if r != col:
                    f = MM[r][col]/MM[col][col]
                    MM[r] = [v - f*w for v, w in zip(MM[r], MM[col])]
        Ainv[i] = [MM[r][p]/MM[r][r] for r in range(p)]
    return coef, [math.sqrt(s2*Ainv[i][i]) for i in range(p)]

random.seed(23)
T = 500
mkt  = [random.gauss(0.0006, 0.012) for _ in range(T)]
hml  = [random.gauss(0.0003, 0.006) for _ in range(T)]
hml2 = [0.99*hml[t] + 0.005*random.gauss(0,1) for t in range(T)]   # near-duplicate (corr~0.73)
y    = [0.02/12 + 1.1*mkt[t] + 0.5*hml[t] + random.gauss(0,0.004) for t in range(T)]

cA, sA = ols_stats([[1.0,mkt[t],hml[t]]  for t in range(T)], y)         # clean FF-style
cB, sB = ols_stats([[1.0,mkt[t],hml[t],hml2[t]] for t in range(T)], y)  # collinear
print("Model A (MKT, HML):        HML beta=%+.4f  se=%.4f  t=%+.2f" % (cA[2], sA[2], cA[2]/sA[2]))
print("Model B (+near-dup HML2):  HML beta=%+.4f  se=%.4f  t=%+.2f" % (cB[2], sB[2], cB[2]/sB[2]))
...
corr(HML, HML2) = 0.731
beta_HML se: 0.0314 (clean) -> 0.0460 (collinear) = 1.5x wider
```
...
3. **The factor zoo / data mining.** Green–Hand–Zhang found ~100 return-predictive characteristics, of which only ~24 carried genuinely independent information — most were redundancy around a handful of real factors. Testing hundreds of candidates guarantees spurious significance by chance; the correct antidote is multiple-testing discipline ([[pillars/01-quantitative-research/backtesting-hygiene-and-deflated-sharpe|Deflated Sharpe Ratio, purged CV]]) and out-of-sample validation.
...
- Sibling: [[pillars/01-quantitative-research/backtesting-hygiene-and-deflated-sharpe|Backtesting Hygiene & Deflated Sharpe]] (multiple-testing) · [[pillars/01-quantitative-research/cross-sectional-and-time-series-momentum|Momentum Crash]] · [[fundamentals-accounting/accounting-quality-and-red-flags/index|Accounting Quality & Red Flags]]
