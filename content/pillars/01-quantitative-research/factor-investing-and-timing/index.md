...
**Basic Prerequisites:** [[foundations/econometrics-and-timeseries/index|Econometrics & Time Series]] (regression, covariance, stationarity) and [[pillars/01-quantitative-research/fundamental-multi-factor-models/index|Fundamental Multi-Factor Models]] (how the factors are *built* — this folder assumes that and asks what happens when you *trade* them).
...
import math, random
random.seed(199)
N, T = 300, 240          # 300 stocks, 20 years monthly
LAM, SIG = 0.003, 0.06   # 0.30%/mo premium per 1 s.d. of characteristic
spreads, ics = [], []
for _ in range(T):
    c = [random.gauss(0, 1) for _ in range(N)]
    r = [LAM*c[i] + random.gauss(0, SIG) for i in range(N)]
    ...
    ics.append(num/math.sqrt(den*sum((r[i]-mr)**2 for i in range(N))))
m = sum(spreads)/len(spreads)
sd = math.sqrt(sum((x-m)**2 for x in spreads)/(len(spreads)-1))
print(f"10-1 spread = {m*100:+.3f}%/mo   IC = {sum(ics)/len(ics):+.3f}   LS Sharpe = {m/sd*math.sqrt(12):.2f}")
```
```
10-1 spread = +1.004%/mo   IC = +0.045   LS Sharpe = 2.05
```
...
- **Absolute beginner:** [[pillars/01-quantitative-research/factor-investing-and-timing/01-from-zero-intuition|01 · From Zero]] — no prior factor knowledge needed.
- **The empirical core (undergrad/job-seeking):** [[pillars/01-quantitative-research/factor-investing-and-timing/02-the-factor-zoo|02 · The Factor Zoo]] → [[pillars/01-quantitative-research/factor-investing-and-timing/03-factor-crowding-and-capacity|03 · Crowding & Capacity]] → [[pillars/01-quantitative-research/factor-investing-and-timing/04-post-publication-decay|04 · Post-Publication Decay]].
- **Robustness & timing (practitioner/graduate):** [[pillars/01-quantitative-research/factor-investing-and-timing/05-failure-modes-and-practice|05 · Failure Modes]] → [[pillars/01-quantitative-research/factor-investing-and-timing/06-advanced-extensions|06 · Advanced Extensions: Factor Timing]].
