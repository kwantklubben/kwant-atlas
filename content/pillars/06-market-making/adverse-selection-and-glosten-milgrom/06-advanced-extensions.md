...
# --- Kyle (1985) one-shot equilibrium ---
sig_v, sig_u = 2.0, 20.0                              # fundamental vol, noise-trader vol
lam = 0.5 * math.sqrt(sig_v**2 / sig_u**2)
print(f"Kyle lambda = 0.5*sqrt(sig_v^2/sig_u^2) = {lam:.4f}   (market depth 1/lambda = {1/lam:.2f})")

random.seed(5); p0 = 100.0
v  = p0 + random.gauss(0, sig_v)                      # true value
x  = (v - p0) / (2 * lam)                             # informed optimal demand
y  = x + random.gauss(0, sig_u)                       # observed flow (x + noise u)
p  = p0 + lam * y                                     # martingale ("semi-strong") price
print(f"one-shot: true v={v:.3f}, informed demand x={x:.3f}, observed flow y={y:.3f}, price p={p:.3f}")
print(f"  signed residual (p-v) = {p-v:+.3f}  -> small: the price already impounds the trade")

# half the private info impounded:  Var[v|y] = Sigma_0/2
beta  = 1.0 / (2 * lam)                               # x = beta*(v - p0), beta = sqrt(sig_u^2/sig_v^2)
cov   = beta * sig_v**2
var_y = beta**2 * sig_v**2 + sig_u**2                 # Var(x) + Var(u)
post  = sig_v**2 - cov**2 / var_y
print(f"Var[v|y] = {post:.4f}  (theory: Sigma_0/2 = {sig_v**2/2:.4f})   -- half the private info impounded")

# --- PIN: the empirical informed probability ---
def pin(alpha, mu, eps): return alpha * mu / (alpha * mu + 2 * eps)
print(f"\nPIN(alpha=.30, mu=10, eps=2) = {pin(0.3, 10, 2):.4f}   (moderate informed activity)")
print(f"PIN(alpha=.30, mu=30, eps=2) = {pin(0.3, 30, 2):.4f}   (heavy informed activity -> higher toxicity)")
```

```text
Kyle lambda = 0.5*sqrt(sig_v^2/sig_u^2) = 0.0500   (market depth 1/lambda = 20.00)
one-shot: true v=97.642, informed demand x=-23.577, observed flow y=-46.540, price p=97.673
  signed residual (p-v) = +0.031  -> small: the price already impounds the trade
Var[v|y] = 2.0000  (theory: Sigma_0/2 = 2.0000)   -- half the private info impounded

PIN(alpha=.30, mu=10, eps=2) = 0.4286   (moderate informed activity)
PIN(alpha=.30, mu=30, eps=2) = 0.6923   (heavy informed activity -> higher toxicity)
```
