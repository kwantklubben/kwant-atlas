...
```python
# --- The five terms of a market maker's day (per share) ---
half_spread = 0.010     # $0.01 captured on each passive fill
rebate      = 0.002     # $0.002/share paid by the exchange to the maker
adverse     = 0.006     # expected adverse-selection loss per share
inventory   = 0.001     # inventory financing/risk cost per share
take_fee    = 0.000     # a passive maker does not pay the take fee

gross = half_spread + rebate
costs = adverse + inventory + take_fee
net   = gross - costs

print(f"gross revenue / fill = {gross:.4f}   (= spread {half_spread} + rebate {rebate})")
print(f"costs         / fill = {costs:.4f}   (= adverse {adverse} + inventory {inventory})")
print(f"net P&L       / fill = {net:+.4f}")
print(f"rebate share of net  = {rebate/net*100:.0f}%")
print(f"break-even half-spread h* = {adverse + inventory - rebate:.4f}")

# scale to a desk: 1,000,000 fills/day x 100 shares
fills, lot = 1_000_000, 100
print(f"desk P&L / day  = ${net * fills * lot:,.0f}")
print(f"without rebate  = ${(net-rebate) * fills * lot:,.0f}  "
      f"(rebate is {(rebate/(net-rebate))*100:.0f}% of P&L)")
```

```text
gross revenue / fill = 0.0120   (= spread 0.01 + rebate 0.002)
costs         / fill = 0.0070   (= adverse 0.006 + inventory 0.001)
net P&L       / fill = +0.0050
rebate share of net  = 40%
break-even half-spread h* = 0.0050
desk P&L / day  = $500,000
