...
gamma, sigma, s, t, T = 0.1, 2.0, 100.0, 0.0, 1.0
tau = T - t

def value(x, s, q):                       # frozen-inventory exponential value (AS eq 2.3)
    return -math.exp(-gamma*x)*math.exp(-gamma*q*s)*math.exp(0.5*gamma**2*q**2*sigma**2*tau)

def reserve_bid(q):                       # solve v(x - rb, s, q+1) = v(x, s, q)  (Def 1, eq 2.4)
    lo, hi = s - 5.0, s + 5.0
    for _ in range(200):
        mid = 0.5*(lo+hi)
        if value(0.0 - mid, s, q+1) > value(0.0, s, q): lo = mid
        else: hi = mid
    return 0.5*(lo+hi)

def reserve_ask(q):                       # solve v(x + ra, s, q-1) = v(x, s, q)  (eq 2.5)
    lo, hi = s - 5.0, s + 5.0
    for _ in range(200):
        mid = 0.5*(lo+hi)
        if value(0.0 + mid, s, q-1) > value(0.0, s, q): hi = mid
        else: lo = mid
    return 0.5*(lo+hi)

for q in (-3, 0, 4):
    rb_num, ra_num = reserve_bid(q), reserve_ask(q)
    rb_f = s + (-1 - 2*q)*gamma*sigma**2*tau/2      # eq 2.7 closed form
    ra_f = s + ( 1 - 2*q)*gamma*sigma**2*tau/2      # eq 2.6 closed form
    r_mid = s - q*gamma*sigma**2*tau                # indifference price
    print(f"q={q:+d}: rb num={rb_num:.4f} vs {rb_f:.4f} | "
          f"ra num={ra_num:.4f} vs {ra_f:.4f} | r-mid={r_mid:.4f}")
```
```
q=-3: rb num=101.0000 vs 101.0000 | ra num=101.4000 vs 101.4000 | r-mid=101.2000
