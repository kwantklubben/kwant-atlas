...
for _ in range(3000):
    ...
    for i in range(n):
        ...
        rate += -(V(pn)-V(p))/(V(p)*dt)   # expected relative loss of pool value per unit time
        cnt  += 1
        p = pn
print(f"measured instantaneous LVR rate = {rate/cnt:.4f}   theory (1/8)sigma^2 = {sig*sig/8:.4f}")
```
```
measured instantaneous LVR rate = 0.4822   theory (1/8)sigma^2 = 0.5000
```
