...
print("Cascade size (of 60) vs shock size  [density=0.15, Cbar=0.05, ebar=0.02]:")
for sh in (0.5, 1.0, 2.0, 4.0):
    s = [cascade(shock=sh, seed=k) for k in range(30)]
    print(f"  shock={sh:4.1f}: mean={np.mean(s):5.2f}/60  p90={np.percentile(s,90):4.1f}  max={max(s)}")

print("\nCascade vs network DENSITY  [shock=4.0]:")
for d in (0.02, 0.05, 0.10, 0.20, 0.40):
    s = [cascade(shock=4.0, density=d, seed=k) for k in range(30)]
    print(f"  density={d:.2f}: mean={np.mean(s):5.2f}/60  max={max(s)}")

print("\nCascade vs dealer CAPITAL buffer Cbar  [shock=4.0, density=0.15]:")
for C in (0.15, 0.10, 0.07, 0.05, 0.03):
    s = [cascade(shock=4.0, Cbar=C, seed=k) for k in range(30)]
    print(f"  Cbar={C:.2f}: mean={np.mean(s):5.2f}/60  max={max(s)}")
```
```text
Cascade size (of 60) vs shock size  [density=0.15, Cbar=0.05, ebar=0.02]:
  shock= 0.5: mean= 1.93/60  p90= 4.0  max=6
  shock= 1.0: mean= 9.33/60  p90=35.0  max=47
  shock= 2.0: mean=49.40/60  p90=60.0  max=60
  shock= 4.0: mean=59.90/60  p90=60.0  max=60

Cascade vs network DENSITY  [shock=4.0]:
  density=0.02: mean= 2.03/60  max=8
  density=0.05: mean=26.20/60  max=51
  density=0.10: mean=56.53/60  max=60
  density=0.20: mean=60.00/60  max=60
  density=0.40: mean=60.00/60  max=60

Cascade vs dealer CAPITAL buffer Cbar  [shock=4.0, density=0.15]:
  Cbar=0.15: mean=21.20/60  max=59
  Cbar=0.10: mean=49.40/60  max=60
  Cbar=0.07: mean=57.70/60  max=60
  Cbar=0.05: mean=59.90/60  max=60
  Cbar=0.03: mean=60.00/60  max=60
```
