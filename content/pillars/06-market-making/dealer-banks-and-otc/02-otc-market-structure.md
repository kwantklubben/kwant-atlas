...
def netting(N=50, density=0.3, seed=1):
    ...
    W = mask*rng.random((N,N))                  # gross bilateral exposures
    gross = W.sum()
    net_i = W.sum(axis=1) - W.sum(axis=0)       # each dealer's net position
    netted = np.abs(net_i).sum()/2              # exposure to the CCP after netting
    return gross, netted

print("Bilateral (gross) vs CCP-netted system exposure, N=50 dealers:")
print(" density   gross    netted    benefit (gross/netted)")
for d in (0.05, 0.10, 0.30, 0.50, 0.80):
    g, n = netting(density=d)
    print(f"  {d:.2f}    {g:7.3f}  {n:7.3f}     {g/n:6.2f}x")
```
```text
Bilateral (gross) vs CCP-netted system exposure, N=50 dealers:
 density   gross    netted    benefit (gross/netted)
