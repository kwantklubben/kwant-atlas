...
def book_impact(dV_list, rho):
    """Sweep a buy imbalance dV through a book with liquidity per unit price
       rho(l) at distance l.  Return the price distance l at which cumulative
       liquidity first covers dV.  rho(l)=l -> diffusive;  rho(l)=1 -> flat."""
    out = []
    for dV in dV_list:
        cum = 0.0; l = 0
        while cum < dV:
...
Q      = [10, 30, 100, 300, 1000, 3000, 10000, 30000, 100000]
Q_flat = [10, 30, 100, 300, 1000, 3000]

imp_diff = book_impact(Q,      rho=lambda l: l)      # diffusive book
imp_flat = book_impact(Q_flat, rho=lambda l: 1.0)    # flat book
print("Impact vs volume imbalance dV (cumulative-liquidity book sweep)")
print("  diffusive book rho(l)=l :", imp_diff)
print(f"     log-log slope = {loglog_slope(Q, imp_diff):.4f}   (square-root law = 0.5)")
print("  flat book      rho(l)=1 :", imp_flat)
print(f"     log-log slope = {loglog_slope(Q_flat, imp_flat):.4f}   (linear impact = 1.0)")
print(f"  analytic check: sqrt(2*dV) at dV=10000 = {math.sqrt(2*10000):.1f}  vs simulated {imp_diff[Q.index(10000)]}")
```

```
Impact vs volume imbalance dV (cumulative-liquidity book sweep)
  diffusive book rho(l)=l : [4, 8, 14, 24, 45, 77, 141, 245, 447]
     log-log slope = 0.5054   (square-root law = 0.5)
  flat book      rho(l)=1 : [10, 30, 100, 300, 1000, 3000]
     log-log slope = 1.0000   (linear impact = 1.0)
  analytic check: sqrt(2*dV) at dV=10000 = 141.4  vs simulated 141
```
