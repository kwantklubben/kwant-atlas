...
# 6 simulated hours. Quiet minutes have ~50 trades; a "news burst" (minutes 180-199)
# has 8x. A TIME clock therefore measures wildly uneven information per bar.
n_minutes = 360
per_min_quiet = 50
burst = range(180, 200)
trades_per_min = [per_min_quiet * 8 if m in burst else per_min_quiet for m in range(n_minutes)]

time_bars_trades = []
for start in range(0, n_minutes, 10):
    time_bars_trades.append(sum(trades_per_min[start:start + 10]))
print("Time-clock bars (10 min): trades per bar =", time_bars_trades[::3])
print("  min =", min(time_bars_trades), " max =", max(time_bars_trades),
      " -> wildly uneven information")

# Volume clock: pack the same trades into fixed-size bars of V = 1000 trades each.
V = 1000
vol_bars = 0
vol_count = 0
for m in range(n_minutes):
    vol_count += trades_per_min[m]
    while vol_count >= V:
        vol_bars += 1
        vol_count -= V
print(f"Volume-clock bars: {vol_bars} bars of V={V} trades each -> constant information per bar")
```
```
Time-clock bars (10 min): trades per bar = [500, 500, 500, 500, 500, 500, 4000, 500, 500, 500, 500, 500]
  min = 500  max = 4000  -> wildly uneven information
Volume-clock bars: 25 bars of V=1000 trades each -> constant information per bar
```
