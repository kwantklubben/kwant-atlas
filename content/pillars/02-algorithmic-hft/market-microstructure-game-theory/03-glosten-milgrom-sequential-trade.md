...
def gm_quotes(theta, pi, VH=101.0, VL=99.0):
    nA = (1+pi)*theta + (1-pi)*(1-theta)
    nB = (1-pi)*theta + (1+pi)*(1-theta)
    A = (VH*(1+pi)*theta + VL*(1-pi)*(1-theta))/nA
    B = (VH*(1-pi)*theta + VL*(1+pi)*(1-theta))/nB
    return A, B, nA, nB
pi, theta = 0.2, 0.5
A, B, nA, nB = gm_quotes(theta, pi)
print(f"Glosten-Milgrom sequential-trade game, pi={pi}, theta={theta}")
print(f"  ask={A:.4f}  bid={B:.4f}  spread={A-B:.4f}   (pi*(VH-VL)={pi*2:.4f})")
th_plus  = theta*(1+pi)/nA; th_minus = theta*(1-pi)/nB
print(f"  theta^+ = theta(1+pi)/nA = {th_plus:.6f}    theta^- = theta(1-pi)/nB = {th_minus:.6f}")
p_now = theta*101 + (1-theta)*99
p_next = 0.5*nA*(th_plus*101+(1-th_plus)*99) + 0.5*nB*(th_minus*101+(1-th_minus)*99)
print(f"  price now = {p_now:.4f}   E[price after one trade] = {p_next:.6f}   drift = {abs(p_next-p_now):.2e}")
print(f"  toxicity P(informed|buy) = 2*pi*theta/nA = {2*pi*theta/nA:.4f}")

def gm_run(pi, Tmax, V, seed):                 # one trading day: V fixed, maker learns
    rnd = random.Random(seed); th = 0.5; prices = [th*101+(1-th)*99]
    for _ in range(Tmax):
        A, B, nA, nB = gm_quotes(th, pi)
        buy = (V == 101.0) if rnd.random() < pi else (rnd.random() < 0.5)
        th = th*(1+pi)/nA if buy else th*(1-pi)/nB
        prices.append(th*101+(1-th)*99)
    return th, prices
convH = [gm_run(pi, 300, 101.0, s)[0] for s in range(400)]
convL = [gm_run(pi, 300,  99.0, s)[0] for s in range(400)]
print(f"  after 300 trades: mean theta | V=VH = {sum(convH)/len(convH):.6f}   mean theta | V=VL = {sum(convL)/len(convL):.6f}")
th, prices = gm_run(pi, 300, 101.0, 0)
print(f"  one path: theta_0=0.500 -> theta_300={th:.6f}   price {prices[0]:.2f} -> {prices[-1]:.4f}")
```
```
Glosten-Milgrom sequential-trade game, pi=0.2, theta=0.5
  ask=100.2000  bid=99.8000  spread=0.4000   (pi*(VH-VL)=0.4000)
  theta^+ = theta(1+pi)/nA = 0.600000    theta^- = theta(1-pi)/nB = 0.400000
  price now = 100.0000   E[price after one trade] = 100.000000   drift = 0.00e+00
  toxicity P(informed|buy) = 2*pi*theta/nA = 0.2000
  after 300 trades: mean theta | V=VH = 0.999971   mean theta | V=VL = 0.000434
  one path: theta_0=0.500 -> theta_300=1.000000   price 100.00 -> 101.0000
```
