...
def ac_trajectory(k):                      # AC eq (17)
    return np.array([X*math.sinh(k*(T-T*j/N))/math.sinh(k*T) for j in range(N+1)])

x = ac_trajectory(kappa)
n = -np.diff(x)
E = 0.5*gamma*X*X + eps*X + etat/tau*np.sum(n**2)   # AC eq (8)
V = sigma*sigma*tau*np.sum(x[1:N]**2)               # AC eq (5)
print(f"kappa = {kappa:.6f}/day   half-life theta = {1/kappa:.4f} days   kappa*T = {kappa*T:.4f}")
print(f"x(1d)={x[int(N*0.2)]:,.0f}   x(T/2)={x[N//2]:,.0f}   x(4d)={x[int(N*0.8)]:,.0f}")
print(f"E[x] = ${E:,.0f}   sd = ${math.sqrt(V):,.0f}")

# cross-check: exact discrete optimum vs the closed form
...
print(f"max |x_numeric - x_closed| = {np.max(np.abs(x_num-x)):.2f} shares  "
      f"({np.max(np.abs(x_num-x))/X:.2e} of X)")
```
```
kappa = 0.601133/day   half-life theta = 1.6635 days   kappa*T = 3.0057
x(1d)=545,055   x(T/2)=212,003   x(4d)=63,324
E[x] = $921,572   sd = $850,375
