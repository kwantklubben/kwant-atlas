...
2. **Determining $\tau$ is market design, not physics.** Shorter intervals restore some speed value ($\delta/\tau$ grows); longer intervals add clearing latency. There is an optimal-interval trade-off (too-short recreates raccing, too-long hurts timeliness) that BCS acknowledge is an open empirical question.
