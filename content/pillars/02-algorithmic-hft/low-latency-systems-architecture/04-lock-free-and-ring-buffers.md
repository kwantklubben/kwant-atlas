...
# Discrete-event SPSC model. A producer emits at fixed cadence (+/- jitter); a
# single consumer serves FIFO. WIP = work accepted but not yet completed -- this
# is exactly what a bounded ring buffer must hold. Overflow -> drop. The consumer
# is either BUSY-POLLING (sees new work instantly) or BLOCKING (an idle consumer
# must be woken: +wakeup_ns each time it had gone to sleep).
def run(capacity, dt_p_ns, med_svc_ns, sigma, n, wakeup_ns, seed=11):
    rnd = random.Random(seed)
    free_at = 0.0
    wip = deque()                       # (finish_time, arrival_time)
    drops = 0; lat = []; max_wip = 0
    for i in range(n):
        a = i * dt_p_ns + rnd.gauss(0, dt_p_ns * 0.05)
        while wip and wip[0][0] <= a:   # record completed work up to 'now'
            fin, arr = wip.popleft(); lat.append(fin - arr)
        if len(wip) >= capacity:
            drops += 1; continue        # ring full -> drop the tick
        idle = free_at < a
        start = (a + wakeup_ns) if (idle and wakeup_ns) else max(free_at, a)
        svc = math.exp(rnd.gauss(math.log(med_svc_ns), sigma))
        free_at = start + svc
        wip.append((free_at, a)); max_wip = max(max_wip, len(wip))
    while wip:
        fin, arr = wip.popleft(); lat.append(fin - arr)
    lat.sort()
    p = lambda x: lat[min(len(lat)-1, int(x*len(lat)))] if lat else 0.0
    return drops, max_wip, (sum(lat)/len(lat) if lat else 0.0), p(0.50), p(0.99)

N = 200_000
print("LOW LOAD (produce every 200ns, service median 60ns)")
print("variant                         drops   max_wip   mean_ns    p50_ns    p99_ns")
for name, wk in (("ring buffer (busy-poll)", 0), ("blocking (wakeup=2us)", 2000)):
    d, md, m, p50, p99 = run(1024, 200.0, 60.0, 0.1, N, wk)
    print(f"{name:30s} {d:6d} {md:9d} {m:9.1f} {p50:9.1f} {p99:9.1f}")

print("\nSUSTAINED OVERLOAD (produce every 100ns, service median 90ns, sigma=0.8"
      " -> mean 124ns)")
print("capacity trades drop rate against queueing latency:")
for C in (16, 64, 256, 1024, 4096):
    d, md, m, p50, p99 = run(C, 100.0, 90.0, 0.8, N, 0)
    print(f"  C={C:5d}: drops={d:6d} ({d/N*100:5.2f}%)  max_wip={md:5d}"
          f"  mean={m:9.1f}ns  p99={p99:10.1f}ns")
```
```
LOW LOAD (produce every 200ns, service median 60ns)
variant                         drops   max_wip   mean_ns    p50_ns    p99_ns
ring buffer (busy-poll)             0         1      60.3      60.0      75.6
blocking (wakeup=2us)               0        11    1086.0    1083.5    2066.5

SUSTAINED OVERLOAD (produce every 100ns, service median 90ns, sigma=0.8 -> mean 124ns)
capacity trades drop rate against queueing latency:
  C=   16: drops= 38491 (19.25%)  max_wip=   16  mean=   1781.6ns  p99=    3174.4ns
  C=   64: drops= 38041 (19.02%)  max_wip=   64  mean=   7696.6ns  p99=   10191.5ns
  C=  256: drops= 37734 (18.87%)  max_wip=  256  mean=  31285.9ns  p99=   35986.7ns
  C= 1024: drops= 37900 (18.95%)  max_wip= 1024  mean= 124947.5ns  p99=  136852.5ns
  C= 4096: drops= 34474 (17.24%)  max_wip= 4096  mean= 474900.3ns  p99=  520920.9ns
```
