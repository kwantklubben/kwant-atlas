...
```python
# 02 - order types: execution on a fixed book (price-time / FIFO)
asks = [(100.05, 300), (100.10, 500), (100.20, 1000)]
bids = [(100.00, 800), (99.95, 1200)]
bb, ba = bids[0][0], asks[0][0]
mid = (bb + ba) / 2

def market_buy(qty):
    rem, filled, notional = qty, 0, 0.0
    for p, q in asks:
        f = min(rem, q); filled += f; notional += f * p; rem -= f
        if rem == 0: break
    return filled, notional, rem

f, n, rem = market_buy(1000)
avg = n / f
print(f"MARKET buy 1000 : filled {f}  avg {avg:.4f}  unexec {rem}")
print(f"   effective spread vs mid = {1e4*(avg-mid)/mid:.2f} bps")
cross = min(1000, asks[0][1])
print(f"LIMIT  buy 1000 @100.05 : fills {cross} @100.05, 700 rest on book")
print(f"   effective spread vs mid = {1e4*(100.05-mid)/mid:.2f} bps")
print(f"IOC    buy 1000 @100.05 : fills 300, cancels 700 (never rests)")
print(f"FOK    buy 1000 @100.05 : REJECTED (available 300 < 1000)")
shown, hidden = 100, 400
print(f"ICEBERG display {shown} of {shown+hidden}: visible tip at 100.05, {hidden} in reserve")
print(f"   reload tranche loses FIFO priority behind orders queued after it")
```
```
MARKET buy 1000 : filled 1000  avg 100.1050  unexec 0
   effective spread vs mid = 8.00 bps
LIMIT  buy 1000 @100.05 : fills 300 @100.05, 700 rest on book
   effective spread vs mid = 2.50 bps
IOC    buy 1000 @100.05 : fills 300, cancels 700 (never rests)
FOK    buy 1000 @100.05 : REJECTED (available 300 < 1000)
ICEBERG display 100 of 500: visible tip at 100.05, 400 in reserve
   reload tranche loses FIFO priority behind orders queued after it
```
