...
```python
# 04 - fees in routing: make/take, the cum-fee spread, and effective cost
take = 0.30/100     # NYSE Arca May 2012: 30 cents per round lot (100 sh) -> $/share
make = 0.21/100     # 21 cents per round lot rebate
print(f"NYSE Arca May 2012: take fee = ${take:.4f}/sh, make rebate = ${make:.4f}/sh, "
      f"venue net = ${take-make:.4f}/sh (make fee = -{make:.4f})")
print(f"SEC cap on take fees (2006) = $0.0030/share (30c / round lot)\n")

orders = [                      # (name, quote, take fee, make fee)
    ("X rebate venue", 100.000, take,   make),
    ("Y free venue",   100.005, 0.0000, 0.0000),
    ("Z low-fee",      100.000, 0.0010, 0.0000),
]
print("100-share marketable buy, all-in (price + take fee):")
for name, px, t, m in orders:
    print(f"  {name:<14} quote {px:.3f}  take {t:.4f}  -> all-in ${px+t:.4f}/sh  = ${100*(px+t):,.2f}")
best = min(orders, key=lambda o: o[1] + o[2])
print(f"  fee-aware SOR picks {best[0]}; a price-only router would tie on X/Z and "
      f"overpay ${100*0.0020:,.2f}\n")

raw = 0.020
print("cum-fee bid-ask spread = raw spread + 2*(take fee):")
for name, t in [("rebate venue", take), ("no-fee venue", 0.0000)]:
    print(f"  {name:<14} raw {raw:.4f}  take {t:.4f}  -> cum-fee spread {raw + 2*t:.4f}")

cum = 0.0260
print(f"\nholding total fee fixed at {make and (take-make):.4f} and cum-fee spread at "
      f"{cum:.4f}, changing the split:")
for t, m in [(0.0030, -0.0021), (0.0020, -0.0011), (0.0010, -0.0001), (0.0009, 0.0000)]:
    tot = t + m                                  # total fee = take + make (make<0 = rebate)
    print(f"  take {t:+.4f} make {m:+.4f} (total {tot:.4f}) -> raw spread must be {cum - 2*t:.4f}")
print("  -> same total fee, same cum-fee spread, but the RAW spread ranges 0.0200-0.0242:")
print("     raw-spread regressions cannot identify the fee split.")

limit = 99.990
print(f"\nmake route: post buy {limit:.3f} on rebate venue -> effective {limit - make:.4f}/sh "
      f"IF filled (vs {best[1]+best[2]:.4f} to take)")
```
```
NYSE Arca May 2012: take fee = $0.0030/sh, make rebate = $0.0021/sh, venue net = $0.0009/sh (make fee = -0.0021)
SEC cap on take fees (2006) = $0.0030/share (30c / round lot)

100-share marketable buy, all-in (price + take fee):
  X rebate venue quote 100.000  take 0.0030  -> all-in $100.0030/sh  = $10,000.30
  Y free venue   quote 100.005  take 0.0000  -> all-in $100.0050/sh  = $10,000.50
  Z low-fee      quote 100.000  take 0.0010  -> all-in $100.0010/sh  = $10,000.10
  fee-aware SOR picks Z low-fee; a price-only router would tie on X/Z and overpay $0.20

cum-fee bid-ask spread = raw spread + 2*(take fee):
  rebate venue   raw 0.0200  take 0.0030  -> cum-fee spread 0.0260
  no-fee venue   raw 0.0200  take 0.0000  -> cum-fee spread 0.0200

holding total fee fixed at 0.0009 and cum-fee spread at 0.0260, changing the split:
  take +0.0030 make -0.0021 (total 0.0009) -> raw spread must be 0.0200
  take +0.0020 make -0.0011 (total 0.0009) -> raw spread must be 0.0220
  take +0.0010 make -0.0001 (total 0.0009) -> raw spread must be 0.0240
  take +0.0009 make +0.0000 (total 0.0009) -> raw spread must be 0.0242
  -> same total fee, same cum-fee spread, but the RAW spread ranges 0.0200-0.0242:
     raw-spread regressions cannot identify the fee split.

make route: post buy 99.990 on rebate venue -> effective 99.9879/sh IF filled (vs 100.0010 to take)
```
