...
```python
# Carbon-exposure engine: TCFD WACI + carbon-price P&L with pass-through
holdings = [
    # name,          w,   revenue($m), scope12(tCO2e), ev_ebitda
    ("Coal utility", 0.10, 2000.0, 8_000_000.0,  6.0),
    ("Oil major",    0.15, 3000.0, 6_000_000.0,  6.0),
    ("Cement",       0.05, 1200.0, 2_400_000.0,  7.0),
    ("Tech",         0.40, 5000.0,   150_000.0, 15.0),
    ("Bank",         0.30, 4000.0,    40_000.0, 10.0),
]
assert abs(sum(w for _, w, _, _, _ in holdings) - 1.0) < 1e-12

waci = sum(w * (E / R) for _, w, R, E, _ in holdings)
mbar = sum(w * M for _, w, _, _, M in holdings)


def carbon_pnl(d_p, lam):
    """Portfolio % value change for a carbon price step d_p ($/tCO2e) and pass-through lam."""
    tot = 0.0
    for _, w, R, E, M in holdings:
        I = E / R                        # tCO2e per $m revenue
        tot += w * (-(1.0 - lam) * M * I * d_p / 1e6)
    return tot


print(f"WACI (TCFD, Scope 1+2)        = {waci:.1f} tCO2e per $m revenue")
print(f"weighted-avg EBITDA multiple  = {mbar:.2f}x")
for d_p in (30.0, 50.0):
    print(f"carbon price +${d_p:.0f}/t, 30% pass-through: portfolio value {100*carbon_pnl(d_p,0.30):+.2f}%")
print(f"pass-through sensitivity, +$30/t: 0% -> {100*carbon_pnl(30.0,0.0):+.2f}%   30% -> {100*carbon_pnl(30.0,0.30):+.2f}%   80% -> {100*carbon_pnl(30.0,0.80):+.2f}%")
```
```
WACI (TCFD, Scope 1+2)        = 815.0 tCO2e per $m revenue
weighted-avg EBITDA multiple  = 10.85x
carbon price +$30/t, 30% pass-through: portfolio value -10.73%
carbon price +$50/t, 30% pass-through: portfolio value -17.88%
pass-through sensitivity, +$30/t: 0% -> -15.33%   30% -> -10.73%   80% -> -3.07%
```
...
- **Absolute beginner:** [[pillars/04-quantitative-risk/climate-and-esg-risk/01-from-zero-intuition|01 · From Zero]] — no prior climate-finance knowledge needed.
- **Metrics + code (undergrad/job-seeking):** [[pillars/04-quantitative-risk/climate-and-esg-risk/02-physical-and-transition-risk|02 · Physical & Transition Risk]] → [[pillars/04-quantitative-risk/climate-and-esg-risk/03-climate-scenarios-and-stress-testing|03 · Climate Scenarios & Stress Testing]] → [[pillars/04-quantitative-risk/climate-and-esg-risk/04-esg-scores-and-the-carbon-premium|04 · ESG Scores & the Carbon Premium]].
- **Robustness (practitioner/graduate):** [[pillars/04-quantitative-risk/climate-and-esg-risk/05-failure-modes-and-practice|05 · Failure Modes]] → [[pillars/04-quantitative-risk/climate-and-esg-risk/06-advanced-extensions|06 · Advanced Extensions (Euler Allocation, Carbon-Adjusted PD)]].
