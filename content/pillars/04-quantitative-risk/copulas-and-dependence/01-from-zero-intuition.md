...
# Two assets, IDENTICAL marginals N(0,1). Only the dependence structure changes.
# Equal-weighted portfolio loss = -(A+B)/2 ; 95% VaR = 0.95-quantile of loss.
def quantile(xs, p):
...
for _ in range(n):
    ...
    ind.append(-(a+b)/2)      # independent copula
    com.append(-(a+a)/2)      # comonotone copula (b=a)
    ctr.append(-(a-a)/2)      # countermonotone copula (b=-a)
print("Same N(0,1) marginals, three dependence structures (equal-weighted 2-asset book)")
for name, s in (("independent",ind),("comonotone",com),("countermonotone",ctr)):
    print(f"  {name:16s}: 95% VaR = {quantile(s,0.95):.4f}   ES95 = {es(s,0.95):.4f}")
print(f"  analytic: ind={1.644854/math.sqrt(2):.4f} com={1.644854:.4f} ctr=0.0000")
```
```
Same N(0,1) marginals, three dependence structures (equal-weighted 2-asset book)
  independent     : 95% VaR = 1.1662   ES95 = 1.4637
  comonotone      : 95% VaR = 1.6431   ES95 = 2.0631
  countermonotone : 95% VaR = 0.0000   ES95 = 0.0000
  analytic: ind=1.1631 com=1.6449 ctr=0.0000
```
