...
2. **Reverse-causality / feedback loops.** A macro stress that lowers capital can itself trigger deleveraging, fire-sales, and further macro decline (the Brunnermeier–Pedersen loss/funding spiral; [[pillars/04-quantitative-risk/liquidity-risk-and-margin-spirals|Liquidity Risk]]). Most bank-level stress tests are *static* (no feedback), so they understate the second round. Supervisors partially compensate with system-wide exercises (e.g. the 2009 SCAP, EBA EU-wide tests).
...
- Sibling: [[pillars/04-quantitative-risk/credit-risk-and-the-merton-model/index|Credit Risk & Merton]] (the default/loss modelling under the macro PD) · [[pillars/04-quantitative-risk/var-and-expected-shortfall/index|VaR & Expected Shortfall]] (ES, ESF/ESR scaling) · [[pillars/04-quantitative-risk/liquidity-risk-and-margin-spirals|Liquidity Risk & Margin Spirals]] (the feedback loop)
- Forward: [[pillars/04-quantitative-risk/index|Pillar 4 · Quantitative Risk]] (capital & model-risk layers that stress feeds into) · [[pillars/04-quantitative-risk/liquidity-risk-and-margin-spirals|Liquidity Risk]] (the funding feedback a macro loss triggers)
