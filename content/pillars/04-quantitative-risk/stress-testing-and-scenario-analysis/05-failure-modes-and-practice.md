...
```python
import math
def pvar(rho): return 0.5**2*0.20**2 + 0.5**2*0.20**2 + 2*0.5*0.5*0.20*0.20*rho
print("rho    port-vol   1yr-99%VaR   diversification reduction")
for rho in (0.3, 0.9):
    s = math.sqrt(pvar(rho))
    var = 2.326*s
    print(f"{rho:.1f}    {s*100:6.2f}%     {var*100:6.2f}%        {(1-s/0.20)*100:5.1f}%")
```
```
rho    port-vol   1yr-99%VaR   diversification reduction
0.3     16.12%      37.51%         19.4%
0.9     19.49%      45.34%          2.5%
```
