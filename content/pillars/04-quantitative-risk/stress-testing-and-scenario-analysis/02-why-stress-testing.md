...
random.seed(7)
n = 1000
rets = sorted(random.gauss(0.0, 0.015) for _ in range(n))   # 1.5% daily vol, normal
var99   = -rets[9]        # 10th worst of 1000 = 1-day 99% VaR
es_tail = -sum(rets[:10])/10.0                                # ES = avg of worst 1%
NAV, B_EQ = 100e6, 0.5
var_loss   = var99*B_EQ*NAV/1e6
es_loss    = es_tail*B_EQ*NAV/1e6
stress_loss = 0.205*B_EQ*NAV/1e6                             # single -20.5% day
print(f"1-day 99% VaR (normal sample) = {var99*100:.2f}%  -> ${var_loss:.2f}M")
print(f"1-day ES (avg of worst 1%)     = {es_tail*100:.2f}%  -> ${es_loss:.2f}M")
print(f"single crisis day (equity -20.5%) = ${stress_loss:.2f}M  ({stress_loss/es_loss:.1f}x ES)")
```
```
1-day 99% VaR (normal sample) = 3.72%  -> $1.86M
1-day ES (avg of worst 1%)     = 4.04%  -> $2.02M
single crisis day (equity -20.5%) = $10.25M  (5.1x ES)
```
