...
def bic(BI):
    if BI <= 1.0:   return 0.12*BI
    if BI <= 30.0:  return 1.0*0.12 + (BI-1.0)*0.15
    return 1.0*0.12 + 29.0*0.15 + (BI-30.0)*0.18
BI, avg_loss = 9.0, 0.06              # EUR bn
BIC = bic(BI);  LC = 15.0*avg_loss
ILM = math.log(math.e - 1.0 + (LC/BIC)**0.8)
ORC = BIC*ILM
print(f"BI={BI} bn -> BIC={BIC:.4f} bn;  LC=15*avg={LC:.4f} bn;  ILM={ILM:.4f}")
print(f"ORC={ORC:.4f} bn;  RWA=12.5*ORC={12.5*ORC:.4f} bn")
print(f"d424 cross-check: BIC(BI=35)={bic(35.0):.2f} (doc: 5.37)")
```
```
BI=9.0 bn -> BIC=1.3200 bn;  LC=15*avg=0.9000 bn;  ILM=0.8979
ORC=1.1852 bn;  RWA=12.5*ORC=14.8149 bn
d424 cross-check: BIC(BI=35)=5.37 (doc: 5.37)
```
...
def poisson(lam):
    ...
    return k-1
random.seed(41)
true_mu, true_sigma, lam2 = 10.0, 0.8, 20.0
def year_loss():
    return sum(math.exp(random.gauss(true_mu,true_sigma)) for _ in range(poisson(lam2)))
M = 200000
S = sorted(year_loss() for _ in range(M))
A, L = 1.0e6, 2.0e6
netS = sorted(s - min(max(s-A,0.0), L-A) for s in S)   # aggregate stop-loss
v0 = S[int(0.999*M)-1]; vn = netS[int(0.999*M)-1]
el0 = sum(S)/M; eln = sum(netS)/M
print(f"Uninsured        : EL={el0:9.0f} VaR99.9={v0:9.0f}")
print(f"Stop-loss A=1M,L=2M: EL={eln:9.0f} VaR99.9={vn:9.0f}  reduction={v0-vn:9.0f} ({100*(v0-vn)/v0:.1f}%)")
```
```
Uninsured        : EL=   606306 VaR99.9=  1361110
Stop-loss A=1M,L=2M: EL=   603103 VaR99.9=  1000000  reduction=   361110 (26.5%)
```
