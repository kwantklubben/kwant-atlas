...
import math, random
random.seed(2026)
# (freq share of ~120 events/yr, lognormal mu, lognormal sigma) per event type
ET = {
 "Internal fraud":                (0.06, 11.0, 1.1),
 "External fraud":                (0.12, 11.5, 1.0),
 "Employment & workplace":        (0.10, 10.5, 0.9),
 "Clients, products & practices": (0.18, 11.0, 1.0),
 "Damage to physical assets":     (0.04, 10.0, 1.2),
 "Business disruption & systems": (0.10, 10.0, 0.9),
 "Execution, delivery & process": (0.40,  9.5, 0.8),
}
def poisson(lam):
    L=math.exp(-lam); k=0; p=1.0
    while p>L: k+=1; p*=random.random()
    return k-1
rows, tot_events, tot_loss = [], 0, 0.0
for name,(share,mu,sig) in ET.items():
    n  = poisson(share*120.0)
    xs = [math.exp(random.gauss(mu,sig)) for _ in range(n)]
    tot_events += n; tot_loss += sum(xs)
    rows.append((name,n,sum(xs)))
print(f"{'event type':<32}{'events':>8}{'loss(EUR)':>14}{'%events':>9}{'%loss':>8}")
for name,n,l in sorted(rows,key=lambda r:-r[2]):
    print(f"{name:<32}{n:>8}{l:>14,.0f}{100*n/tot_events:>8.1f}%{100*l/tot_loss:>7.1f}%")
print(f"\nTOTAL {tot_events} events, EUR {tot_loss:,.0f}")
```
```
event type                        events     loss(EUR)  %events   %loss
Clients, products & practices         31     2,656,202    23.0%   38.9%
External fraud                        18     1,837,880    13.3%   26.9%
Execution, delivery & process         57       942,020    42.2%   13.8%
Employment & workplace                 9       560,134     6.7%    8.2%
Business disruption & systems         10       476,762     7.4%    7.0%
Internal fraud                         5       188,342     3.7%    2.8%
Damage to physical assets              5       170,008     3.7%    2.5%
