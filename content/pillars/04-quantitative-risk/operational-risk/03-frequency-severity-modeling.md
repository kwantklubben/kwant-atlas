...
def fit_lognormal_moments(m, v):
    """Method-of-moments fit: recover (mu, sigma) of Lognormal from sample mean m, var v."""
    s2 = math.log(1.0 + v/(m*m))
    return math.log(m) - 0.5*s2, math.sqrt(s2)

true_mu, true_sigma = 10.0, 0.8     # unknown to the modeler in reality
lam = 20.0                          # mean losses per year
N = 300                             # observed individual loss events
losses = [math.exp(random.gauss(true_mu, true_sigma)) for _ in range(N)]
m = sum(losses)/N
v = sum((x-m)**2 for x in losses)/(N-1)
fit_mu, fit_sigma = fit_lognormal_moments(m, v)
theo = math.exp(true_mu + 0.5*true_sigma**2)
fit  = math.exp(fit_mu + 0.5*fit_sigma**2)
print(f"sample mean={m:.1f}  sample var={v:.1f}")
print(f"fitted Lognormal: mu={fit_mu:.4f} sigma={fit_sigma:.4f}")
print(f"theoretical E[X]={theo:.1f}   fitted E[X]={fit:.1f}")
print(f"Expected annual loss EL = lambda*E[X] = {lam*fit:.0f}")
```
```
sample mean=31243.0  sample var=778482306.6
fitted Lognormal: mu=10.0563 sigma=0.7658
theoretical E[X]=30333.3   fitted E[X]=31243.0
Expected annual loss EL = lambda*E[X] = 624860
```
