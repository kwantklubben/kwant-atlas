---
title: "01 — Liquidity Risk from Zero: Two Kinds of 'Can't Sell'"
tags:
...
```python
# --- market liquidity: you sell at the BID, not the mid ---
Q, mid, bid, ask = 250_000, 50.00, 49.95, 50.05
half_spread = (ask - bid) / 2
spread_cost = Q * half_spread
print(f"Position {Q:,} shares, mid ${mid:.2f}, quote {bid:.2f}/{ask:.2f}")
print(f"half-spread = ${half_spread:.3f}/share -> market-liquidity cost = ${spread_cost:,.0f}"
      f"  ({spread_cost/(Q*mid)*1e4:.1f} bp of {Q*mid:,.0f})")

# --- funding liquidity: haircut sets the maximum leverage ---
collateral = 1_000_000.0
for h in (0.02, 0.10, 0.25, 0.50):
    loan = collateral * (1 - h)
    print(f"haircut {h*100:4.0f}%  -> cash raised ${loan:,.0f}  -> leverage {collateral/(collateral-loan):.2f}x")
```
```
Position 250,000 shares, mid $50.00, quote 49.95/50.05
half-spread = $0.050/share -> market-liquidity cost = $12,500  (10.0 bp of 12,500,000)
haircut    2%  -> cash raised $980,000  -> leverage 50.00x
haircut   10%  -> cash raised $900,000  -> leverage 10.00x
haircut   25%  -> cash raised $750,000  -> leverage 4.00x
haircut   50%  -> cash raised $500,000  -> leverage 2.00x
```
