...
1. **Low power of backtests.** With $n=250$ and $p=0.01$, the Kupiec test needs roughly $\ge7$ exceptions to reject at $5\%$ (see the hub table: $LR_{uc}$ first exceeds the $5\%$ critical value $3.84$ at $x=7$). A model can be materially wrong (true rate $2\%$) and still pass. *Passing a backtest is weak evidence; failing is strong evidence.*
