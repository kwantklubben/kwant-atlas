...
def hist_var(sample, a):
    s = sorted(sample)
    return s[min(len(s)-1, int(a*len(s)))]

def normal_var(sample, a):
    m = sum(sample)/len(sample)
    v = sum((x-m)**2 for x in sample)/(len(sample)-1)
    return m + math.sqrt(v)*Phi_inv(a)

def pot_var(sample, a, thr=0.95):
    s = sorted(sample); N = len(s); u = s[int(thr*N)]
    exc = [x-u for x in s if x > u]; Nu = len(exc)
    mbar = sum(exc)/Nu
    s2 = sum((x-mbar)**2 for x in exc)/(Nu-1)
    xi  = 0.5*(1 - mbar*mbar/s2)                 # method-of-moments GPD shape
    beta= 0.5*mbar*(mbar*mbar/s2 + 1)            # GPD scale
    return u + (beta/xi)*(((N/Nu)*(1-a))**(-xi) - 1)

# N=2000 returns, Student-t(5) scaled to unit variance
random.seed(77); N = 2000; smp = []
for _ in range(N):
    z = random.gauss(0,1)
    chi = sum(random.gauss(0,1)**2 for _ in range(5))
    t = z/math.sqrt(chi/5.0)
    smp.append(t*math.sqrt(3.0/5.0))

for a in (0.99, 0.995, 0.999):
    nv, hv, pv = normal_var(smp,a), hist_var(smp,a), pot_var(smp,a)
    lo, hi = min(nv,hv,pv), max(nv,hv,pv)
    print(f"alpha={a:.3f}  normal={nv:.4f}  historical={hv:.4f}  POT-GPD={pv:.4f}  "
          f"spread={100*(hi-lo)/lo:.1f}%")
```
```
alpha=0.990  normal=2.2581  historical=2.4296  POT-GPD=2.4070  spread=7.6%
alpha=0.995  normal=2.5045  historical=2.9538  POT-GPD=2.8693  spread=17.9%
alpha=0.999  normal=3.0128  historical=4.2414  POT-GPD=4.1194  spread=40.8%
```
