...
# (a) ESTIMATION ERROR: truth p=0.52, you estimate phat=0.57 (+1sd from n=100)
p_true, n, phat = 0.52, 100, 0.57
sd = math.sqrt(p_true*0.48/n)
f_hat = 2*phat - 1                       # bet full Kelly on your estimate
f_true = 2*p_true - 1                    # true optimum
g_true = lambda f: p_true*math.log(1+f) + (1-p_true)*math.log(1-f)
print(f"std(p-hat) = {sd:.4f}")
print(f"bet f*=f(phat)={f_hat:.4f}  but true Kelly={f_true:.4f}  -> overbet")
print(f"true g at your bet = {g_true(f_hat):+.5f}   true g at true Kelly = {g_true(f_true):+.5f}")

def sim_over(f, steps=2000, paths=40000, seed=3):
    random.seed(seed); ruin = 0; logs = 0.0
    for _ in range(paths):
        x = 1.0
        for _ in range(steps):
            x *= (1+f) if random.random() < p_true else (1-f)
        logs += math.log(max(x,1e-300)); ruin += (x < 0.01)
    return math.exp(logs/paths), ruin/paths
for f, lab in [(f_true, "true-Kelly  f=0.04"), (f_hat, "overbet     f=0.14")]:
    mw, ru = sim_over(f)
    print(f"{lab}: median wealth after 2000 = {mw:8.4f}   P(bankroll<1%) = {ru:.3f}")

# (b) FAT TAILS: +20% w.p. .6, -5% w.p. .3, -60% w.p. .1
rets = [0.20, -0.05, -0.60]; probs = [0.6, 0.3, 0.1]
mu  = sum(pr*r for pr, r in zip(probs, rets))
var = sum(pr*r*r for pr, r in zip(probs, rets)) - mu*mu
g_ex = lambda f: sum(pr*math.log(1+f*r) for pr, r in zip(probs, rets))
best, bg = 0.0, -1e9
for i in range(1001):
    v = g_ex(i/1000)
    if v > bg: bg, best = v, i/1000
print(f"\nheavy tail: Gaussian-approx f* = mu/s^2 = {mu/var:.4f}   exact log-opt f* = {best:.3f}")
```
```text
std(p-hat) = 0.0500
