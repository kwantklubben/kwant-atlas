...
# (1) discrete even-money
p = 0.55; fstar_disc = p - (1-p)
g = lambda f: p*math.log(1+f) + (1-p)*math.log(1-f)
print(f"even-money: f* = p-q = {fstar_disc:.4f}   g(f*) = {g(fstar_disc):.6f}")

# (2) discrete unequal payoff: f* = m/(ab), m = bp - aq
p2, b2, a2 = 0.40, 2.0, 1.0
m2 = b2*p2 - a2*(1-p2)
print(f"unequal (b=2,a=1,p=.4): m={m2:.4f}  f*=m/(ab)={m2/(a2*b2):.4f}")

# (3) critical fraction of the even-money game: bisect g(f)=0
lo, hi = 1e-12, 1-1e-12
for _ in range(300):
    mid = (lo+hi)/2
    if g(mid) > 0: lo = mid
    else: hi = mid
print(f"critical f_c (g=0, p=.55) = {lo:.6f}")

# (4) continuous / securities
m, s, r = 0.11, 0.15, 0.06
fk = (m-r)/s**2
gstar = r + fk*(m-r) - 0.5*s**2*fk**2
print(f"continuous: f*=(m-r)/s^2 = {fk:.4f}   g(f*) = {gstar:.6f}  (= S^2/2+r = {((m-r)/s)**2/2+r:.6f})")
fc_hi, lo_c = 1e-6, 0.0
gc = lambda f: r + f*(m-r) - 0.5*s**2*f**2
L, H = 1e-6, 1000.0
for _ in range(200):
    mid = (L+H)/2
    if gc(mid) > 0: L = mid
    else: H = mid
print(f"continuous critical f_c (upper root) = {L:.4f}")
```
```text
even-money: f* = p-q = 0.1000   g(f*) = 0.005008
unequal (b=2,a=1,p=.4): m=0.2000  f*=m/(ab)=0.1000
critical f_c (g=0, p=.55) = 0.198668
continuous: f*=(m-r)/s^2 = 2.2222   g(f*) = 0.115556  (= S^2/2+r = 0.115556)
