...
p, q = 0.55, 0.45
def g(f): return p*math.log(1+f) + q*math.log(1-f)   # per-round growth rate

print("growth-rate landscape g(f):")
for f in (0.00, 0.10, 0.20, 0.30, 0.50, 0.70, 0.90):
    print(f"  f={f:.2f}:  g={g(f):+.5f}")

def sim(f, n=200, paths=40000, seed=1):
    random.seed(seed); logs = 0.0; ruin = 0
    for _ in range(paths):
        x = 1.0
        for _ in range(n):
            x *= (1+f) if random.random() < p else (1-f)
        logs += math.log(max(x, 1e-300)); ruin += (x < 1e-6)
    return math.exp(logs/paths), ruin/paths

for f, lab in ((0.10, "Kelly f=0.10"), (1.0, "all-in f=1.0")):
    med, ru = sim(f)
    print(f"{lab}: median wealth after 200 bets = {med:8.3f}   P(ruin) = {ru:.3f}")
```
```text
growth-rate landscape g(f):
  f=0.00:  g=+0.00000
  f=0.10:  g=+0.00501
  f=0.20:  g=-0.00014
  f=0.30:  g=-0.01620
  ...
  f=0.70:  g=-0.24994
  f=0.90:  g=-0.68314
Kelly f=0.10: median wealth after 200 bets =    2.725   P(ruin) = 0.000
all-in f=1.0: median wealth after 200 bets =    0.000   P(ruin) = 1.000
```
