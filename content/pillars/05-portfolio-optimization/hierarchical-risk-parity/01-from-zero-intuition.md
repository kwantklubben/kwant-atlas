...
def link_single(dist):
    n = dist.shape[0]; d = np.full((2*n-1, 2*n-1), np.inf); d[:n, :n] = dist
    ...
    for _ in range(n - 1):
        ...
        Z.append([i, j, h]); active = [k for k in active if k not in (i, j)] + [new]
    return np.array(Z)
...
vols = np.array([0.10, 0.11, 0.30])
R = np.array([[1.0, 0.9, 0.1], [0.9, 1.0, 0.1], [0.1, 0.1, 1.0]])
cov = R * np.outer(vols, vols)
d = corr_dist(cov)
print("correlation distance matrix:")
for row in d: print("  " + " ".join(f"{x:.4f}" for x in row))
w, order = hrp(cov)
print("tree leaf order:", order)
print("HRP weights:", [round(float(x), 4) for x in w])
print("1/N weights :", [0.3333]*3)
print("portfolio vol HRP = %.6f  1/N = %.6f"
      % (np.sqrt(w @ cov @ w), np.sqrt(np.ones(3)/3 @ cov @ (np.ones(3)/3))))
```
```
correlation distance matrix:
  0.0000 0.2236 0.6708
  0.2236 0.0000 0.6708
  0.6708 0.6708 0.0000
tree leaf order: [2, 0, 1]
HRP weights: [0.4909, 0.4057, 0.1034]
1/N weights : [0.3333, 0.3333, 0.3333]
portfolio vol HRP = 0.099441  1/N = 0.126711
```
