...
print("unconstrained naive:", np.round(w_naive,3), " gross=%.2f"%np.abs(w_naive).sum())

# --- long-only / capped MVO via projected gradient (a QP solver stand-in) ---
def qp(lo, hi, iters=80000, step=0.02):
    w = np.ones(N)/N
    for _ in range(iters):
        w = np.clip(w - step*(-mu_s + delta*(S_s @ w)), lo, hi)
        s = w.sum(); w = w/s if s > 0 else w
    return w
print("long-only        :", np.round(qp(0.0, 1.00),3))
print("long-only cap .35:", np.round(qp(0.0, 0.35),3))

# --- Michaud resampling: optimize on B resamples, then average ---
def resample_disp(B=500, seed=1):
    rr = np.random.RandomState(seed); W = np.zeros((B,N)); Ls = np.linalg.cholesky(S_s)
    for b in range(B):
        X = (Ls @ rr.randn(N,T)).T + mu_s          # parametric resample around the estimate
        W[b] = (1.0/delta)*np.linalg.solve(np.cov(X,rowvar=False), X.mean(0))
    return W
W = resample_disp()
print("\nMichaud averaged weights :", np.round(W.mean(0),3), " gross=%.2f"%np.abs(W.mean(0)).sum())
print("resample weight sd       :", np.round(W.std(0),3))
print("gross per draw: mean=%.2f  sd=%.2f"%(np.abs(W).sum(1).mean(), np.abs(W).sum(1).std()))
```
```
unconstrained naive: [-2.257  3.33   0.28   0.78   0.299  4.963]  gross=11.91
long-only        : [0.078 0.158 0.205 0.214 0.171 0.173]
long-only cap .35: [0.078 0.158 0.205 0.214 0.171 0.173]

Michaud averaged weights : [-2.668  3.673  0.404  0.975  0.368  5.833]  gross=13.92
resample weight sd       : [1.936 2.783 1.411 1.325 1.833 2.912]
gross per draw: mean=16.45  sd=5.49
```
