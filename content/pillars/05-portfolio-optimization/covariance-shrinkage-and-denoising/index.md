...
def lw_identity(X):
    ...
    mu = np.trace(S) / N                    # grand mean of eigenvalues
    F  = mu * np.eye(N)                     # structured target
    dif = X[:, :, None] * X[:, None, :] - S # (T,N,N) residuals x_it*x_jt - s_ij
    pi  = (dif**2).mean(axis=0).sum()                      # sum_ij Var(s_ij) scale
    rho = np.sum((dif**2).mean(axis=0)[np.arange(N), np.arange(N)])  # trace part
    gamma = ((F - S)**2).sum()                             # ||F - S||_F^2
    delta = min(max((pi - rho) / gamma / T, 0.0), 1.0)     # optimal intensity
    return delta * F + (1 - delta) * S, delta, mu

# --- verify: error maximization on a small N>T-free but hostile problem ---
...
print(f"delta*={delta:.4f}  mu={mu:.6f}")
print(f"cond(S)={np.linalg.cond(S):.3f}  cond(LW)={np.linalg.cond(S_lw):.3f}")
```
```
delta*=0.3734  mu=0.673152
cond(S)=56.769  cond(LW)=6.133
```
