...
4. **Shrinkage is not a substitute for the return estimate.** $\hat\Sigma$ fixes the risk side only. The dominant MVO input error is in $\mu$ (Chopra & Ziemba 1993: mean errors dominate covariance errors by ~10×) — see [[pillars/05-portfolio-optimization/black-litterman/index|Black–Litterman]].
