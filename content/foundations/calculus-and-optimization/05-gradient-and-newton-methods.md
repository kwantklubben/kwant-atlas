...
# (A) coupled quadratic, condition number M/m = 20
A = [[10.5, 9.5], [9.5, 10.5]]      # eigenvalues 20 and 1
b = [20.0, 20.0]                    # optimum at x* = (1,1)
xs = [1.0, 1.0]
def f(x):  return 0.5*(A[0][0]*x[0]*x[0] + 2*A[0][1]*x[0]*x[1] + A[1][1]*x[1]*x[1]) - b[0]*x[0] - b[1]*x[1]
def gf(x): return [A[0][0]*x[0]+A[0][1]*x[1]-b[0], A[1][0]*x[0]+A[1][1]*x[1]-b[1]]
m, M, t = 1.0, 20.0, 1.0/20.0
x, k = [2.0, 1.0], 0
print("(A) gradient descent, cond(A)=20, step=1/M, c=1-m/M=0.95")
print(f"    bound (eq. 9.19): log(1e6)/log(1/c) = {math.log(1e6)/math.log(1/(1-m/M)):.0f} iterations")
while f(x)-f(xs) > 1e-6 and k < 5000:
    g = gf(x); x = [x[i]-t*g[i] for i in range(2)]; k += 1
print(f"    reached f-f* < 1e-6 in k={k} iterations;  f-f* = {f(x)-f(xs):.2e}")
print(f"    final x = {[round(v,8) for v in x]}")

# (B) Newton on the same quadratic -> exact in one step
def solve2(H, g):
    det = H[0][0]*H[1][1] - H[0][1]*H[1][0]
    return [(-H[1][1]*g[0] + H[0][1]*g[1])/det, (H[1][0]*g[0] - H[0][0]*g[1])/det]
x = [2.0, 1.0]; d = solve2(A, gf(x)); x = [x[i]+d[i] for i in range(2)]
print(f"(B) Newton step dx = -grad^2 f(x)^-1 grad f(x): x -> {[round(v,10) for v in x]}  (exact, 1 iteration)")

# (C) quadratic convergence on non-quadratic f(x)=exp(x)+x^2
fn = lambda x: math.exp(x) + x*x
d1 = lambda x: math.exp(x) + 2*x
d2 = lambda x: math.exp(x) + 2.0
lo, hi = -1.0, 0.0
for _ in range(200):
    mid = 0.5*(lo+hi)
    if d1(mid) > 0: hi = mid
...
xstar = 0.5*(lo+hi)
print(f"\n(C) Newton on f=exp(x)+x^2,  x* = {xstar:.12f}")
x = 0.0
for i in range(5):
    print(f"    iter {i}: |x-x*| = {abs(x-xstar):.3e}")
    x = x - d1(x)/d2(x)
```
```
(A) gradient descent, cond(A)=20, step=1/M, c=1-m/M=0.95
    bound (eq. 9.19): log(1e6)/log(1/c) = 269 iterations
    reached f-f* < 1e-6 in k=122 iterations;  f-f* = 9.17e-07
    final x = [1.00095774, 0.99904226]
(B) Newton step dx = -grad^2 f(x)^-1 grad f(x): x -> [1.0, 1.0]  (exact, 1 iteration)

(C) Newton on f=exp(x)+x^2,  x* = -0.351733711249
    iter 0: |x-x*| = 3.517e-01
    iter 1: |x-x*| = 1.840e-02
    iter 2: |x-x*| = 4.438e-05
    iter 3: |x-x*| = 2.563e-10
    iter 4: |x-x*| = 5.551e-17
```

**Read the output.** (A) Gradient descent takes $122$ iterations to reach $f-f^\*<10^{-6}$ on a condition-number-$20$ problem — far more than Newton's *one*, and already in the regime where the exact method counts (the bound of $269$ is worst-case, so the measured count is *better* than the guarantee, but the order of magnitude — "hundreds of cheap steps versus one expensive solve" — is the real comparison). (B) Newton lands exactly on $x^\*=(1,1)$ in a single step, because for a quadratic the model *is* the function. (C) On a non-quadratic, Newton's error sequence is $3.5\times10^{-1},\,1.8\times10^{-2},\,4.4\times10^{-5},\,2.6\times10^{-10},\,5.6\times10^{-17}$ — each iteration squares the previous error (roughly: $1.8\times10^{-2}\to(1.8\times10^{-2})^2\approx3\times10^{-4}\to$ and so on down), i.e. **quadratic convergence**, reaching machine precision in four steps.
...
- Applied: [[foundations/numerical-methods/04-numerical-optimization|Numerical Optimization]] (the methods as numerical primitives) · [[pillars/07-machine-learning-altdata/index|Machine Learning]] (SGD, IRLS, gradient boosting) · [[pillars/05-portfolio-optimization/modern-portfolio-theory-and-mean-variance|Mean-Variance Optimization]]
