...
1. **The non-PSD crash is a *diagnosis*, not a bug.** Cholesky of a non-PSD covariance hits $\sqrt{\text{negative}}$ — the routine throws (`Matrix is not positive definite`). That exception is telling you the covariance violates $\lambda_i\ge0$, which happens with $N>T$, pairwise-assembled correlations, or missing-data covariance estimates. Fix the covariance ([[foundations/linear-algebra-and-matrices/06-advanced-extensions|06]], [[pillars/05-portfolio-optimization/covariance-shrinkage-and-denoising|shrinkage]]), not the solver.
