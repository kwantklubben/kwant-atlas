...
- *Vasicek:* $dR=(\alpha-\beta R)dt+\sigma dW$ ⇒ $R(t)=e^{-\beta t}R(0)+\tfrac{\alpha}{\beta}(1-e^{-\beta t})+\sigma e^{-\beta t}\!\int_0^t e^{\beta s}dW(s)$, Gaussian, can go negative.
- *CIR:* $dR=(\alpha-\beta R)dt+\sigma\sqrt R\,dW$ — no closed form, nonnegative, **Feller condition** $2\alpha\beta\ge\sigma^2$ for strict positivity (Glasserman §3.4).
...
4. **Feller condition ignored.** Below $2\alpha\beta<\sigma^2$ a CIR Euler scheme crosses below zero; Milstein or exact noncentral-$\chi^2$ sampling is required. See [[foundations/stochastic-calculus/04-sdes-and-simulation|04 · SDEs & Simulation]].
